...
KLIB is callable from any task page via the standard `CALL24` mechanism, and from kernel code via the same. Kernel code that predates KLIB (e.g. `_RawPutDec` in `kos_splash.asm`) calls the underlying implementations (`_KMul16x16_32`, `_KDiv10`) by their ROM addresses directly; user code must use the jump table.
...
| 02 | `KLIB_DIVMOD16` | stub | Math |
| 03 | `KLIB_UDIVMOD16` | stub | Math |
| 04 | `KLIB_DIVMOD32` | stub | Math |
...
The current `KLIB_VERSION_VALUE` is `$0100` (high byte = major, low byte = minor).
...
|---|---|---|
| 02 | `KLIB_DIVMOD16` | Signed 16/16 -> 16 quotient + 16 remainder |
| 03 | `KLIB_UDIVMOD16` | Unsigned 16/16 -> 16 + 16 |
| 04 | `KLIB_DIVMOD32` | 32/16 -> 32 + 16 |
| 05 | `KLIB_MUL32x32_32` | 32x32 -> low 32 multiply |

Implementations deferred until needed by a real consumer (kosh, file system, etc.). Plan: shift-and-subtract / shift-and-add bit algorithms, not the Forth-style repeated-subtraction approach.
...
Signed 16-bit integer to ASCII decimal.

```
In:       D0       signed value (-32768..32767)
          XY0      destination buffer (at least 7 bytes for "-32768\0")
Out:      D0       byte count written (excluding nul); 1..6
          XY0      unchanged
Clobbers: D1, D2, D3
Flags:    C = 0
```

Always emits a leading `-` for negative values. Always nul-terminates the result.

### 7.2 `KLIB_UTOA` (slot 41)

Unsigned 16-bit integer to ASCII decimal.

```
In:       D0       unsigned value (0..65535)
          XY0      destination buffer (at least 6 bytes)
Out:      D0       byte count written (excluding nul); 1..5
          XY0      unchanged
Clobbers: D1, D2, D3
Flags:    C = 0
```
...
Signed 16-bit integer to ASCII hex, fixed 4-digit format.

```
In:       D0       16-bit value (interpreted bitwise; sign is irrelevant)
          XY0      destination buffer (at least 5 bytes for "FFFF\0")
Out:      D0       always 4
          XY0      unchanged
Clobbers: D1, D2
Flags:    C = 0
...
Always 4 hex digits, uppercase, no `$` prefix, no leading-zero suppression. The output is purely the underlying bit pattern -- for example, `-1` is written as `FFFF`.
...
*End of k/OS KLIB Reference Manual v1.0*
