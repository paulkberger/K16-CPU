...
Standard shift-subtract algorithm: 32 iterations of left-shifting a 33-bit register `(R:HI:LO)` and trial-subtracting the divisor from `R` when `R >= divisor`. Loop body uses a 3-word `ADD/ADC/ADC` chain to shift the 33-bit register left by one — this is necessary because the K16's `SHL`/`ROL` are LOOKUP-based and don't propagate carry (see Reference Manual §6.5 vs §6.3). Worst-case ~1500-1800 cycles. The divisor and bit counter live on the stack across the loop to free a D-register for the running remainder.
...
*End of k/OS KLIB Reference Manual v1.2*
