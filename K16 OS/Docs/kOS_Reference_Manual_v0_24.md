...
| `kos_fs.asm` / `kos_fs_dir.asm` | Directory layer + Phase 2a path resolver (`_Resolve`/`_ResolveParent`) and the subdirectory syscalls `sys_mkdir` (#69), `sys_resolve` (#70), `sys_pwd` (#71), `sys_rmdir` (#72) |
...
| `kosh_cmds_fs.asm` | vol, ls, cat, format, run, cp (Part 25 r1), rm + mv (Part 25 r2), load (Part 25 r6), mkdir + cd + pwd + rmdir (Part 44) |
...
Slots in per-group reserve (19..24, 33..39, 42..49, 54..59, 73..74, 78..84) and the Reserved tail (85..127) are uninitialised at boot and resolve to the bad-trap handler, which prints a diagnostic and halts.
...
Column order is `TID PTID NAME ST FG GFX PAGE BLOCKS BYTES TICKS`.
...
*End of k/OS Reference Manual v0.24*
