; ============================================================================
; SCROLL2.asm  --  visual test of gfx_scroll, up then down  (.COM)
; ----------------------------------------------------------------------------
...
.loop:
                ...
                ; --- Phase 2: scroll DOWN (reverse path; feeds at top) ---
                LOADI   D0, #SCROLL_LINES
                STOREP  D0, Y3, [#sc_count]
.dloop:
                LOADI   D0, #WIN_X
                ...
                LOADI   D0, #WIN_H
                STOREP  D0, Y3, [#gr_h]
                LOADI   D0, #LINE_H             ; dv = +LINE_H (down)
                CALLR   gfx_scroll
                CALLR   feed_top
                ...
                ; restore text mode (release video ownership) before exit,
                ; else the graphics panel stays up over the shell prompt
                LOADI   D0, #0
...
; ============================================================================
; feed_top -- clear the exposed TOP band and draw the next table line there
;   (used by the down-scroll phase). Advances the feed cursor (wraps).
; ============================================================================
feed_top:
                LOADI   D0, #WIN_X
                STOREP  D0, Y3, [#gr_x]
...
; ----------------------------------------------------------------------------
banner          .TEXT   "SCROLL2: gfx_scroll up+down auto-scroll demo", 0
msg_busy        .TEXT   "gfx: graphics busy", 0
win_title       .TEXT   "k/OS scroll test  -  80x25  -  gfx_scroll", 0
prompt          .TEXT   "auto-scroll: up 60, then down 60, then exit", 0
