...
; ============================================================================
; Date:    17 May 2026
; Status:  Phase 14 Part 3b — ps now shows heap usage per task.
;
; Revision: r8 — 17 May 2026 — Phase 14 Part 3b: BLOCKS+BYTES columns
...
.do_ps:
                ; Header
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ps_hdr - kosh_entry)
                TRAP    #TRAP_PUTS
...
.ps_loop:
                ...
                ; --- Row indent ---------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ps_indent - kosh_entry)
                TRAP    #TRAP_PUTS
...
.ps_name_done:
                ...
                LOW     D0
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (state_chars - kosh_entry)
                ADD     X0, D0
                LOADB   D0, [XY0]
                TRAP    #TRAP_PUTCHAR
                LOADI   D0, #CH_SPACE
...
.kill_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_kill_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.kill_bad_arg:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_kill_badarg - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.kill_err:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_kill_err_pfx - kosh_entry)
...
.do_info:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ver_full - kosh_entry)
                TRAP    #TRAP_PUTS
...
.info_active_done:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_info_ticks - kosh_entry)
                TRAP    #TRAP_PUTS
...
.do_task:
                ...
                BNE.S   .tcb_have_args
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.tcb_addr_done:
                ...
                ; Slot is free.
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_unused - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.tcb_in_use:
                ; --- "tcb N:" header ---------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_hdr - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                ...
                TRAP    #TRAP_PUTDEC
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_colon - kosh_entry)
                TRAP    #TRAP_PUTS

                ; --- state -------------------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_state - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_page - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                ; --- saved x (stack ptr) -----------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_savedx - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                ; --- parent ------------------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_parent - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                ; --- yields ------------------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_yields - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                ; --- preempts ----------------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_preempts - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADI   Y0, #$00
                LOADP   D0, Y3, [#DUMP_PAGE]
                MOVE    X0, D0
                ...
                ; --- name --------------------------------------------------
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_tcb_name - kosh_entry)
                TRAP    #TRAP_PUTS
...
; --- ver --------------------------------------------------------------------
msg_ver_full:  .TEXT   "Shell:    kosh v0.8 (k/OS Phase 16.7)\n",0
