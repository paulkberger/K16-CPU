...
; ============================================================================
; Date:    29 May 2026
; Status:  Part 38 - cleanup: dead per-error strings removed (cp/cat/mv)
...
;             • Uses CALL24 _HostFOpen / _HostFRead / _HostFClose
...
;
;   .INCLUDEd from kosh.asm between kosh_entry: and kosh_entry_end:
;   so the page-relative addressing convention
...
.do_vol:
                ; Emit header line.
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_vol_hdr - kosh_entry)
                TRAP    #TRAP_PUTS

                ; Loop drives 0..FS_MAX_DRIVES-1.
                LOADI   D0, #0
                STOREZ  D0, [#DISK_DRIVE_TMP]
.dv_loop:
                LOADZ   D0, [#DISK_DRIVE_TMP]
                CMP     D0, #FS_MAX_DRIVES
                BHS.S   .dv_done
                CALL24  _KoshPrintVolLine       ; D0 = drive index
                LOADZ   D0, [#DISK_DRIVE_TMP]
                ADD     D0, #1
                STOREZ  D0, [#DISK_DRIVE_TMP]
                BRA     .dv_loop
...
.ls_check_arg:
                ...
                CALL24  _KoshSplitDrivePat      ; D0=drive, XY1=basename ptr
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (ls_star_pat - kosh_entry)
                STOREZ  X0, [#LS_PAT_PTR]
                BRA     .ls_have_drive
.ls_pat_explicit:
                ; Pattern = basename pointer (XY1).
                MOVE    D0, X1
                STOREZ  D0, [#LS_PAT_PTR]
                BRA     .ls_have_drive
...
.ls_hdr_lbl_loop:
                ...
                LOADI   D0, #0
                STOREZ  D0, [#LS_FILE_COUNT]
                STOREZ  D0, [#LS_TOTAL_LO]
                STOREZ  D0, [#LS_TOTAL_HI]

                STOREZ  D2, [#LS_DRIVE_TMP]
                LOADI   D3, #0                  ; D3 = index
                STOREZ  D3, [#LS_INDEX_TMP]

.ls_loop:
                ; Re-load drive + index (might have been clobbered last iter
                ; by the row-build helpers).
                LOADZ   D0, [#LS_DRIVE_TMP]
                LOADZ   D1, [#LS_INDEX_TMP]
                MOVE    Y0, Y3
                ...
                LOADZ   D0, [#LS_TOTAL_LO]
                LOADZ   D1, [#LS_TOTAL_HI]
                ADD     D0, D2                  ; lo += size_lo, sets C
                ADC     D1, D3                  ; hi += size_hi + C
                STOREZ  D0, [#LS_TOTAL_LO]
                STOREZ  D1, [#LS_TOTAL_HI]
...
.ls_after_size:
                ...
                ; Tally and advance.
                LOADZ   D0, [#LS_FILE_COUNT]
                ADD     D0, #1
                STOREZ  D0, [#LS_FILE_COUNT]

                LOADZ   D0, [#LS_INDEX_TMP]
                ADD     D0, #1
                STOREZ  D0, [#LS_INDEX_TMP]
                BRA     .ls_loop
...
.ls_done:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ls_files - kosh_entry)
                ...
                LOADZ   D1, [#LS_TOTAL_HI]
                LOADI   D2, #0                  ; raw mode
                CALL24  _KoshEmitSize

                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ls_used - kosh_entry)
                CALL24  _KoshEmitStrZ
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ls_free - kosh_entry)
                CALL24  _KoshEmitStrZ
                BRA.S   .ls_emit_row
...
.ls_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_ls_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cat_literal_err:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cat_openerr - kosh_entry)
...
.cat_glob:
                CALL24  _KoshSplitDrivePat      ; D0=drive, XY1=basename pattern
                BCS     .cat_glob_baddrv
                ...
                SHL     D3
                ADD     D0, D3                  ; *14
                STOREZ  D0, [#GLOB_RSVSIZE]
                SUB     X3, D0
                MOVE    D0, X3
                STOREZ  D0, [#GLOB_TABLE]

                ; Expand.
                LOADZ   D0, [#GLOB_DRIVE]
                CALL24  _KoshGlobExpand
...
.cat_glob_release:
                LOADZ   D0, [#GLOB_RSVSIZE]
                ADD     X3, D0
                BRA     .repl_loop

.cat_glob_baddrv:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_drivewild - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cat_glob_nomatch:
                ...
                ADD     X3, D0
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_nomatch - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cat_glob_toomany:
                ...
                ADD     X3, D0
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_toomany - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.cat_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cat_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.format_label_start:
                ...
                MOVE    Y1, Y3
                LOADI   X1, #SPAWN_ENTRY_OFFSET + (kosh_format_label - kosh_entry)
...
.format_lbl_done:
                ...
                MOVE    Y1, Y3
                LOADI   X1, #SPAWN_ENTRY_OFFSET + (kosh_format_custom - kosh_entry)
                LOADI   D0, #1
                STOREB  D0, [XY1]
                BRA     .format_issue
...
.format_default_label:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (kosh_format_label - kosh_entry)
                CALL24  _HostBayName
...
.format_issue:
                ...
                MOVE    Y1, Y3
                LOADI   X1, #SPAWN_ENTRY_OFFSET + (kosh_format_label + 10 - kosh_entry)
...
.format_print_ok:
                ; Success. Print "OK\n".
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_ok - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.format_rename_warn:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_rename_warn - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.format_bad_label:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_badlabel - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.format_no_arg:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.format_bad_drive:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_baddrv - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.fmt_failed:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_format_failed - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.run_no_bg:
                ...
                CALL24  _KoshEmitByte
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_run_exit_lbl - kosh_entry)
                CALL24  _KoshEmitStrZ
                MOVE    D0, D2
                CALL24  _KoshEmitDec
                LOADI   D0, #']'
...
.run_bg_report:
                ...
                CALL24  _KoshEmitByte
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_run_bg_lbl - kosh_entry)
                CALL24  _KoshEmitStrZ
                MOVE    D0, D2
                CALL24  _KoshEmitDec
                LOADI   D0, #']'
...
.run_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_run_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.run_exec_err:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_run_execerr - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.run_wait_err:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_run_waiterr - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.cp_dst_terminated:
                ...
                CALL24  _KoshExpandBareDst

                CALL24  _KoshCpOne
                ; C=0 OK, C=1 error (D0=err, or special codes - see worker).
                BCS     .cp_report_err
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_ok - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cp_report_err:
                ...
                BEQ     .cp_exists_msg
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_writeerr - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop

.cp_exists_msg:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_exists - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cp_glob:
                ...
                LOADZ   D0, [#CP_DST_PATH_TMP]
                MOVE    X0, D0
                CALL24  _KoshSplitDrivePat      ; D0=destdrive, XY1=remainder
                BCS     .cp_glob_baddrv
                ...
                ; Stash dest drive.
                LOADZ   D0, [#CP_DST_PATH_TMP]  ; (re-derive drive below cleanly)
                MOVE    Y0, Y3
                LOADZ   D0, [#CP_DST_PATH_TMP]
                MOVE    X0, D0
                CALL24  _KoshSplitDrivePat
                STOREZ  D0, [#CP_DSTDRV_TMP]    ; dest drive index

                ; Split SRC into drive + pattern.
                MOVE    Y0, Y3
                LOADZ   D0, [#CP_SRC_PATH_TMP]
                MOVE    X0, D0
                CALL24  _KoshSplitDrivePat      ; D0=srcdrive, XY1=pattern
                BCS     .cp_glob_baddrv
                STOREZ  D0, [#GLOB_DRIVE]
...
.cp_glob_iter:
                ...
                LOADZ   D0, [#GLOB_TABLE]
                ADD     D3, D0                  ; D3 = name source offset
                STOREZ  D3, [#CP_NAME_TMP]      ; remember for both src and dst
...
.cp_glob_dstdone:
                ...
                TRAP    #TRAP_PUTS
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_arrow - kosh_entry)
                TRAP    #TRAP_PUTS
                MOVE    Y0, Y3
...
.cp_glob_item_same:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_samepath - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cp_glob_destwild:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_destwild - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.cp_glob_multidest:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_multidest - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.cp_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_cp_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.rm_glob:
                ...
                STOREZ  D0, [#GLOB_DRIVE]       ; (also re-stashed inside expander)
                ...
                ; Iterate matches.
                STOREZ  D0, [#GLOB_COUNT]       ; total
                LOADI   D0, #0
                STOREZ  D0, [#GLOB_INDEX]       ; iterator
...
.rm_glob_namedone:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rm_failed - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .rm_glob_advance
...
.rm_glob_ok_one:
                ...
                TRAP    #TRAP_PUTS
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_glob_removed - kosh_entry)
                TRAP    #TRAP_PUTS
...
.rm_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rm_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mv_report_err:
                ...
                BEQ     .mv_exists_msg
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mv_failed - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.mv_ro_note:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mv_ro_src - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mv_exists_msg:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mv_dst_exists - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mv_glob_check_warn:
                ...
                TRAP    #TRAP_PUTS
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mv_ro_src_tag - kosh_entry)
                TRAP    #TRAP_PUTS
                LOADZ   D0, [#GLOB_INDEX]
                ADD     D0, #1
                STOREZ  D0, [#GLOB_INDEX]
...
.mv_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mv_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.load_name_terminated:
                ...
                TRAP    #TRAP_CLOSE
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_exists - kosh_entry)
                TRAP    #TRAP_PUTS
...
.load_open_dest_for_write:
                ...
                STOREZ  D0, [#LOAD_DST_FD_TMP]

                ; Part 34: initialise cumulative bytes-written counter to 0.
                LOADI   D0, #0
                STOREZ  D0, [#LOAD_WRITTEN_LO]
                STOREZ  D0, [#LOAD_WRITTEN_HI]
...
.load_copy_loop:
                ...
                LOADZ   D1, [#LOAD_WRITTEN_LO]
                ADD     D1, D0                  ; D1 = new low; C = carry
                STOREZ  D1, [#LOAD_WRITTEN_LO]
                LOADZ   D1, [#LOAD_WRITTEN_HI]
                ADC     D1, #0                  ; propagate carry
                STOREZ  D1, [#LOAD_WRITTEN_HI]
...
.load_eof:
                ...
                ; Emit "loaded " prefix.
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_ok - kosh_entry)
                CALL24  _KoshEmitStrZ

                ; Emit decimal byte count.
                LOADZ   D0, [#LOAD_SIZE_TMP]
                CALL24  _KoshEmitDec

                ; Emit " bytes\n" suffix.
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_bytes - kosh_entry)
                CALL24  _KoshEmitStrZ
...
.load_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.load_fopen_err:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_fopen_err - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.load_create_err:
                ...
                MOVE    D0, D3
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_create_err - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.load_fread_err:
                ...
                MOVE    D0, D3
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_fread_err - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.load_write_err:
                ...
                ; rather than D3.
                STOREZ  D0, [#LOAD_ERR_TMP]

                ; Add partial-chunk count (D1) to cumulative.
                LOADZ   D0, [#LOAD_WRITTEN_LO]
                ADD     D0, D1
                STOREZ  D0, [#LOAD_WRITTEN_LO]
                LOADZ   D0, [#LOAD_WRITTEN_HI]
                ADC     D0, #0
                STOREZ  D0, [#LOAD_WRITTEN_HI]
                ...
                LOADI   X1, #ROW_BUF
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_wrote - kosh_entry)
                CALL24  _KoshEmitStrZ
                LOADZ   D0, [#LOAD_WRITTEN_LO]
                LOADZ   D1, [#LOAD_WRITTEN_HI]
                ...
                CALL24  _KoshEmitSize
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_wrote_then - kosh_entry)
                CALL24  _KoshEmitStrZ
                LOADI   D0, #0
                CALL24  _KoshEmitByte
                MOVE    Y0, Y3
                ...
                ; stashed err code.
                LOADZ   D0, [#LOAD_ERR_TMP]
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_write_err - kosh_entry)
                CALL24  _KoshPrintErr
                BRA     .repl_loop
...
.load_short_write:
                ...
                CALL24  _HostFClose
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_load_short_write - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
_KoshPrintVolLine:
                ...
                CALL24  _KoshEmitByte
                CALL24  _KoshEmitByte
                CALL24  _KoshEmitByte           ; 5 trailing spaces, col0 = 7
                ...
                STOREZ  D0, [#VOL_FREE_TMP]
                STOREZ  D1, [#VOL_TOTAL_TMP]
                STOREZ  D2, [#VOL_CLSZ_TMP]
...
.pvl_unmounted:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_vol_unfmt_pre - kosh_entry)
                CALL24  _KoshEmitStrZ
                ...
                ; Emit ")".
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_vol_unfmt_post - kosh_entry)
                CALL24  _KoshEmitStrZ
                BRA     .pvl_um_finish

.pvl_um_notmounted:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_vol_unmounted - kosh_entry)
                CALL24  _KoshEmitStrZ
