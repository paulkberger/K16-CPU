...
; ============================================================================
; Date:    11 May 2026
; Status:  Part 25 r5 — remount command for external-modification refresh.
;
...
; so SPAWN_ENTRY_OFFSET + (label - kosh_entry) addressing works.
;
; --- Scratch usage --------------------------------------------------------
...
.do_disks:
                ...
                LOADI   D0, #LIST_BUF
                STOREZ  D0, [#DISK_WALK_TMP]

.disks_loop:
                ; Re-load walk pointer (Y3 = task page; X = offset).
                MOVE    Y0, Y3
                LOADZ   D0, [#DISK_WALK_TMP]
                MOVE    X0, D0
...
.disks_no_host:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_disks_unavail - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mount_no_lc:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_disks_ok - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mount_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mount_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mount_baddrv:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mount_baddrv - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mount_host_err:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mount_err - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mount_tryerr:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mount_badbpb - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.unm_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_unmount_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.unm_err:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_unmount_err - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.mk_have_size:
                ...
                STOREZ  D0, [#DISK_SECTORS_TMP]

                LEA     XY0, XY1
                LOADZ   D0, [#DISK_SECTORS_TMP]
                CALL24  _HostCreate
                BCS     .mk_create_err
...
.mk_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mkdisk_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mk_badsize:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mkdisk_badsize - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mk_toosmall:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mkdisk_toosmall - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.mk_create_err:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_mkdisk_err - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.rm_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rmdisk_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.rm_err:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rmdisk_err - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.rn_have_name:
                ...
                MOVE    Y1, Y3
                LOADI   X1, #SPAWN_ENTRY_OFFSET + (kosh_rename_buf - kosh_entry)
...
.rn_copy_done:
                ...
                SUB     D0, #DSK_HOST_BAY_FIRST_DRV     ; bay 0..3
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (kosh_rename_buf - kosh_entry)
                CALL24  _HostRename
                BCS     .rn_failed
...
.rn_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rename_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.rn_baddrv:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rename_baddrv - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.rn_badname:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rename_badname - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.rn_failed:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_rename_err - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.rem_usage:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_remount_usage - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop

.rem_baddrv:
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_remount_baddrv - kosh_entry)
                TRAP    #TRAP_PUTS
                BRA     .repl_loop
...
.rem_failed:
                ...
                MOVE    Y0, Y3
                LOADI   X0, #SPAWN_ENTRY_OFFSET + (msg_remount_failed - kosh_entry)
