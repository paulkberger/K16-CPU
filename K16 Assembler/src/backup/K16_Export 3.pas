...
begin
  ...
  { Assembler emits big-endian words — swap to little-endian for standard Intel HEX }
  j := 0;
  while j + 1 < Length(BinImage) do
  begin
    i           := BinImage[j];
    BinImage[j]     := BinImage[j + 1];
    BinImage[j + 1] := i;
    Inc(j, 2);
  end;
  ...
  Result := Format('Saved %d bytes at $%06X as Intel HEX', [Length(BinImage), MinAddr]);
end;
