...
type
  ...
  private
    ...
    // Post-pass lint: warn on INC/DEC XYn immediately followed by a
    // conditional branch — INC/DEC XY trashes all four flags (gotcha 3).
    procedure WarnIncDecBeforeBranch;
...
procedure TK16Assembler.WarnIncDecBeforeBranch;
// ISA gotcha 3: INC/DEC XYn (XY-pair forms) trash all four flags (C, Z, N, V)
// in hardware. If the next instruction is a conditional branch, it's almost
// certainly using stale flags from before the INC/DEC — a latent bug. Emit a
// warning. Register-only INC/DEC (Dn, Xn, Yn) do set flags normally and are
// not warned about.
//
// The check operates on FInstructions (post-FirstPass). We pair each
// INC/DEC XYn with the next real instruction (skipping labels/directives
// that emit no code).
var
  i, j: Integer;
  Instr, Next: TInstructionRecord;
  IsIncDecXY: Boolean;
  IsCondBranch: Boolean;
  Up: string;
  OpUp: string;
begin
  for i := 0 to FInstructions.Count - 2 do
  begin
    Instr := FInstructions[i];
    if (not SameText(Instr.Mnemonic, 'INC')) and
       (not SameText(Instr.Mnemonic, 'DEC')) then
      Continue;

    // Operand 0 must start with "XY"
    if (Length(Instr.Operands) < 1) or
       (Length(Instr.Operands[0]) < 2) then
      Continue;
    Up := UpperCase(Copy(Instr.Operands[0], 1, 2));
    IsIncDecXY := (Up = 'XY');
    if not IsIncDecXY then Continue;

    // Find next real instruction (skip records that are directives/no-ops
    // — conservatively, anything with a Mnemonic starting with '.').
    j := i + 1;
    while j < FInstructions.Count do
    begin
      Next := FInstructions[j];
      if (Length(Next.Mnemonic) > 0) and (Next.Mnemonic[1] <> '.') then
        Break;
      Inc(j);
    end;
    if j >= FInstructions.Count then Continue;

    OpUp := UpperCase(Next.Mnemonic);
    IsCondBranch :=
      (OpUp = 'BEQ') or (OpUp = 'BNE') or
      (OpUp = 'BCS') or (OpUp = 'BCC') or
      (OpUp = 'BMI') or (OpUp = 'BPL') or
      (OpUp = 'BVS') or (OpUp = 'BVC') or
      (OpUp = 'BGE') or (OpUp = 'BLT') or
      (OpUp = 'BGT') or (OpUp = 'BLE') or
      (OpUp = 'BHI') or (OpUp = 'BLS') or
      (OpUp = 'BEQ.L') or (OpUp = 'BNE.L') or
      (OpUp = 'BCS.L') or (OpUp = 'BCC.L') or
      (OpUp = 'BMI.L') or (OpUp = 'BPL.L') or
      (OpUp = 'BVS.L') or (OpUp = 'BVC.L') or
      (OpUp = 'BGE.L') or (OpUp = 'BLT.L') or
      (OpUp = 'BGT.L') or (OpUp = 'BLE.L') or
      (OpUp = 'BHI.L') or (OpUp = 'BLS.L');

    if IsCondBranch then
      AddWarning(Format('%s %s immediately before %s: flags are trashed by ' +
                        'INC/DEC XY (gotcha 3); branch uses stale flags',
                        [Instr.Mnemonic, Instr.Operands[0], Next.Mnemonic]),
                 Next.LineNumber);
  end;
end;
...
// Address-aware binary writer. Builds a byte-granular sparse image from
// FMachineCode by OR-ing each record's bytes into the image at its correct
// address, then serialises the dense range. This tolerates:
//   (a) odd-address records produced by .BYTE when PC is odd
//   (b) multiple records at the same word address (OR semantics)
//   (c) gaps between records (filled with $00)
// Image origin is FBaseAddress (set by .BASE), or the lowest record address
// if .BASE was never used.
var
  BinaryFile: TFileStream;
  i: Integer;
  MachCode: TMachineCode;
  Bytes: TArray<Byte>;
  MinAddr, MaxAddr, ImageSize, Offset: UInt32;
  Image: TBytes;
  j: Integer;
begin
  if FMachineCode.Count = 0 then
  begin
    // Empty output — create a zero-length file for consistency.
    BinaryFile := TFileStream.Create(Filename, fmCreate);
    BinaryFile.Free;
    Exit;
  end;

  // First pass: determine image extent.
  MinAddr := $FFFFFFFF;
  MaxAddr := 0;
  for i := 0 to FMachineCode.Count - 1 do
  begin
    ...
    if (MachCode.Address + UInt32(Length(Bytes)) - 1) > MaxAddr then
      MaxAddr := MachCode.Address + UInt32(Length(Bytes)) - 1;
  end;

  // Image origin: FBaseAddress if set (<= MinAddr), otherwise MinAddr itself.
  // Using FBaseAddress lets the user pin the start of the image file to a
  // known ROM base even if the first record is above that.
  if (FBaseAddress > 0) and (FBaseAddress <= MinAddr) then
    MinAddr := FBaseAddress;

  ImageSize := (MaxAddr - MinAddr) + 1;
  SetLength(Image, ImageSize);
  FillChar(Image[0], ImageSize, 0);

  // Second pass: OR each record's bytes into the image at its address.
  // OR (not assign) so that .BYTE patching an existing word's high half
  // composes correctly even when records overlap.
  for i := 0 to FMachineCode.Count - 1 do
  begin
    MachCode := FMachineCode[i];
    Bytes := MachCode.GetBytes;
    if Length(Bytes) = 0 then Continue;
    Offset := MachCode.Address - MinAddr;
    for j := 0 to High(Bytes) do
      Image[Offset + UInt32(j)] := Image[Offset + UInt32(j)] or Bytes[j];
  end;
  ...
  try
    if ImageSize > 0 then
      BinaryFile.WriteBuffer(Image[0], ImageSize);
  finally
    BinaryFile.Free;
  end;
end;
