...
begin
  ...
  begin
    ...
    // Use long mode unless the target is forward and within +31 bytes.
    // Backward branches and far-forward branches use long form.
    TempOffset := Integer(TargetAddress) - (Integer(Instr.Address) + 2);
    IsLongMode := (TempOffset < 0) or (TempOffset > 31);
