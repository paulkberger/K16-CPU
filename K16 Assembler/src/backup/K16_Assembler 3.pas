...
var
  ...
  // IsSigned is set by TImmediateValue.Parse when '#-N' syntax is used.
  if Instr.Immediate.IsSigned and not Instr.Immediate.IsSymbol then
  begin
    ErrorReporter(Format(
      'Negative immediate #-%d is not valid for %s — use the positive-immediate form (e.g. SUB instead of ADD with negative)',
      [Instr.Immediate.Value, Instr.Mnemonic]), Instr.LineNumber);
    Result.OpCode  := 0;
    Result.Address := Instr.Address;
    Exit;
  end;
