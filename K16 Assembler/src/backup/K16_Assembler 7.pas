...
begin
  ...
  // [PC+imm16] is always 2 words (4 bytes). Do NOT rely on the dummy encode:
  // for forward references DummySizeSymbolResolver returns 1000, which produces
  // a wildly out-of-range RelOffset vs a $FFxxxx base address. The encoder's
  // range validator then exits early leaving HasImmediate=False, so GetTotalWords
  // returns 1 instead of 2. Affects LOADD/LOADB/LOADX/LOADY/STORED/STOREB mode 10
  // and LEA. Fix: amPCRel is unconditionally 2 words — no encoder call needed.
  if Instr.Mode = amPCRel then
  begin
    Result := 2;
    Exit;
  end;
