...
{
  K16_Encoder_TrapRet.pas — Encoder for TRAP, NEG, RET, RETCC, RETCS instructions.

  All five use opcode $1E. NEG was relocated here from $00 mode 11 by
  CR-2026-001 v1.2 (FLAGSX change) — at $00 its flag writes would have
  been suppressed by the FLAGSX hardware, breaking its arithmetic
  contract. RETCC and RETCS share mode 10 to economise on opcode/mode
  slots; they are discriminated by IR[8:7] (read by the control ROM in
  step 5).

    Mode 00                TRAP #n             Software trap / syscall
    Mode 01                NEG dst[, src]      Two's complement negate
  ...
  NEG encoding (operand field layout preserved from old $00 mode 11):
    IR[8:5] = dst register (4 bits, D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11)
    IR[4:1] = src register (4 bits, same encoding)
    IR[0]   = 0 (not connected to ROM A0)
    NEG dst        — single-operand form: src = dst (in-place)
    NEG dst, src   — two-operand form
}
...
const
  ...
  NEG_BASE       = $F200;   // opcode $1E, mode 01
...
begin
  Result := ['TRAP', 'NEG', 'RET', 'RETCC', 'RETCS'];
end;
...
begin
  ...
  begin
    // NEG dst[, src] — opcode $1E mode 01 (relocated from $00 mode 11
    // by CR-2026-001 v1.2). Two-operand form: dst <- -src.
    // Single-operand form (NEG dst) is sugar for NEG dst, dst.
    // Operand field layout preserved bit-for-bit from old encoding:
    //   IR[8:5] = dst, IR[4:1] = src, IR[0] = 0
    if not (Instr.Destination.RegType in [rtData, rtIndexX, rtIndexY]) then
    begin
      ErrorReporter('NEG: invalid destination register (must be D0-D3, X0-X3, or Y0-Y3)',
                    Instr.LineNumber);
    end
    else
    begin
      ...
      begin
        ErrorReporter('NEG: invalid operand format — use NEG dst or NEG dst, src',
                      Instr.LineNumber);
      end
      else
      begin
  ...
  begin
    // RETCC [#nw] — opcode $1E mode 10, IR[8:7]=00  →  C=0 on return
  ...
  begin
    // RETCS [#nw] — opcode $1E mode 10, IR[8:7]=01  →  C=1 on return
