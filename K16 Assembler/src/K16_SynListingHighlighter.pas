...
// Code word: 4 hex chars in one chunk (e.g. "C2E0").
// Byte data:  pairs of 2 hex chars separated by spaces (e.g. "48 65 6C 6C ...").
// Distinguish by peeking: if char[2] is a space and char[3] is hex, it's
// byte data — consume the whole run as plain identifier (black) and skip
// straight to the source column.
begin
  if (fLine[Run + 2] = ' ') and IsHexChar(fLine[Run + 3]) then
  begin
    // Byte-data line — consume all "XX XX XX ..." pairs (and the spaces
    // between them) as one black token. Stop when we hit anything that
    // isn't either a hex pair or an interior space.
    fTokenID := tkIdentifier;
    while True do
    begin
      // Must be at the start of a hex pair
      if not (IsHexChar(fLine[Run]) and IsHexChar(fLine[Run + 1])) then
        Break;
      Inc(Run, 2);
      // Peek: another pair after a single space?
      if (fLine[Run] = ' ') and
         IsHexChar(fLine[Run + 1]) and IsHexChar(fLine[Run + 2]) then
        Inc(Run)   // swallow inter-byte space
      else
        Break;     // run of bytes ended
    end;
    // Byte-data lines have no immediate / opmode columns — go to source.
    fLineState := llsSource;
  end
  else
  begin
