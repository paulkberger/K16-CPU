...

{
  ...
  Syntax for hardware INC/DEC (24-bit, opcode $02):
    ...
    DEC XYn, #imm     ; XYn -= imm (0-31) — step is mandatory
    INC XYn           ; ASSEMBLER ERROR: explicit step required
    DEC XYn           ; ASSEMBLER ERROR: explicit step required
...
begin
  ...
  begin
    ...
    begin
      // Bare INC/DEC XYn is an error — step size must be explicit
      ErrorReporter(Format('%s XY%d requires an explicit step: use #Nb (byte) or #Nw (word), e.g. %s XY%d, #1',
        [Instr.Mnemonic, XYn, Instr.Mnemonic, XYn]), Instr.LineNumber);
      Result.OpCode := 0;
      Exit;
    end;
