...
begin
  ...
  Settings.WriteString('ROMDrive','File', Trim(FileNameEditROMdisk.FileName));
...
begin
  ...
  outPath := Trim(DirectoryEdit.Text);
  if outPath <> '' then
    outPath := IncludeTrailingPathDelimiter(outPath);
  s := GenerateROMs( K16Assembler, outPath, CheckBoxAddLookups.Checked,
                     Trim(FileNameEditROMdisk.FileName) );
  MemoOutput.Lines.Add( s );
end;
