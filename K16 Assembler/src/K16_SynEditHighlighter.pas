...
type
  ...
  private
    ...
    function PeekHasSecondOperand: Boolean;
...
const
  ...
  // SHL/SHR/ASR are NOT listed here — they're native LOOKUP ops in
  // K16_INSTRUCTIONS and get re-tagged as sugar at parse time in
  // IdentProc when a second operand is present (the two-operand pseudo
  // form). This avoids mis-colouring the native one-operand uses.
...
// Peek ahead to see whether this instruction has a second operand (i.e.
// there's a `,` somewhere on the rest of the line before a comment or
// end-of-line). Used to distinguish two-operand pseudo forms like
// "SHL Dn, #count" from native one-operand "SHL Dn".
// K16 instructions use ',' only as an operand separator — addressing
// modes use '+' inside brackets. We do not consume anything.
function TSynK16Syn.PeekHasSecondOperand: Boolean;
var
  p: LongInt;
begin
  p := Run;
  while not (fLine[p] in [#0, #10, #13, ';']) do
  begin
    if fLine[p] = ',' then Exit(True);
    Inc(p);
  end;
  Result := False;
end;
...
begin
  ...
  begin
    ...
    // SHL/SHR/ASR are native one-operand LOOKUP ops, but the assembler
    // also accepts two-operand pseudo forms (SHL Dn, #count). When a
    // comma follows on the same line, re-tag as sugar to signal "this
    // is the pseudo form" — matches the colour given to BHI/BLS and
    // other pseudo-instructions.
    if ((UpperCase(Token) = 'SHL') or (UpperCase(Token) = 'SHR') or
        (UpperCase(Token) = 'ASR')) and PeekHasSecondOperand then
      fTokenID := tkSugar;
