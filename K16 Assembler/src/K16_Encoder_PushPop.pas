...
type
  ...
  private
    ...
    function IsObsoleteGroup(const Name: string): Boolean;
...
begin
  // v3.12: D123 is the only valid group mnemonic.
  // The old D / X / Y group mnemonics are no longer accepted; they are
  // detected separately in Encode() so a helpful migration error can be
  // emitted.
  Result := SameText(Name, 'D123');
end;

function TK16PushPopEncoder.IsObsoleteGroup(const Name: string): Boolean;
begin
  // Old group mnemonics removed in v3.12. Detected for migration error.
...
begin
  // v3.12: D123 is the only valid group, encoded with IR[8:5] = 0000
  // (same bit pattern previously used for the all-four D group).
  if SameText(GroupName, 'D123') then Result := 0
  else Result := 255; // Invalid
end;
...
begin
  ...
  else if IsObsoleteGroup(FirstOperand) then
  begin
    // v3.12: PUSH D / POP D removed; opcode/mode slot reassigned to D123
    // (D1, D2, D3 only — V2 ABI callee-saved set; D0 not touched).
    // PUSH X / POP X and PUSH Y / POP Y group mnemonics were never
    // emitted by the previous encoder and are also rejected.
    if SameText(FirstOperand, 'D') then
      ErrorReporter(
        Format('%s D removed in manual v3.12 — use %s D123 (D1, D2, D3) ' +
               'or individual %s instructions if D0 must also be saved',
               [Instr.Mnemonic, Instr.Mnemonic, Instr.Mnemonic]),
        Instr.LineNumber)
    else
      ErrorReporter(
        Format('%s %s group not supported — use individual ' +
               'register %s instructions',
               [Instr.Mnemonic, FirstOperand, Instr.Mnemonic]),
        Instr.LineNumber);
    Result.OpCode := 0;
    Exit;
  end
  ...
  begin
    // MODE 01: Register group (v3.12 — D123 only)
    // D123 pushes/pops D1, D2, D3 in order (D0 not touched).
    // Encoding is identical to the previous PUSH D / POP D encoding;
    // the microcode has been replaced to skip the D0 transfer steps.
