...
begin
  ...
  begin
    ...
    { Banner uses the user's source text verbatim (including their quotes
      and any subdir/path prefix) so it matches the END marker emitted by
      the listing generator.  ExtractDirectiveContent returns the text
      after ".INCBIN", trim-stripped. }
    ModifiedInstr.PseudoBanner :=
      Format('=== BEGIN INCBIN %s (%d bytes, %d words) ===',
             [Trim(ExtractDirectiveContent(Instr.SourceLine, '.INCBIN')),
              Length(IncBinData),
              Length(IncBinData) div 2]);
    FInstructions.Add(ModifiedInstr);
    FInstrScope.Add(FCurrentGlobalLabel);
