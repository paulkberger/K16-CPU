...
type
  // -------------------------------------------------------------------
  // Pseudo-instruction support  (see K16_Assembler_Pseudo_Instruction_Spec_v1_0.md)
  // -------------------------------------------------------------------
  // The parser recognises pseudo-mnemonics (BHI, BLS, and the
  // two-operand forms SHL/SHR/ASR Dn,#n) and tags the resulting
  // TInstructionRecord with PseudoKind so FirstPass can dispatch to
  // K16_PseudoBranch / K16_PseudoShift for expansion.  No expansion
  // happens in the parser — only recognition.
  TPseudoKind = (
    pkNone,         // not a pseudo (normal instruction or directive)
    pkBHI,          // Branch if Higher  (unsigned strict)
    pkBLS,          // Branch if Lower or Same  (unsigned)
    pkSHL_N,        // SHL Dn, #count
    pkSHR_N,        // SHR Dn, #count
    pkASR_N         // ASR Dn, #count
  );

  // Size hint for branch pseudos (BHI/BLS).  Set from the .S / .L
  // mnemonic suffix; bshAuto if neither is present.  Kept here (not in
  // K16_PseudoCommon) so the parser can write it without circular
  // unit references.
  TParsedBranchSize = (pbsAuto, pbsShort, pbsLong);
  ...
  TImmediateValue = record
    ...
    // WARNING (audit 22 April 2026): the IsValid*Bit predicates return True
    // for a zero-filled record (Value=0, IsSigned=False, IsSymbol=False).
    // Every current caller guards by checking immediate presence separately
    // (e.g. only calling IsValid* immediately after Parse), so the trap is
    // latent. Do NOT rely on IsValid*Bit alone to mean "has an immediate";
    // pair it with an explicit presence check. If a future change needs a
    // proper "has immediate" flag, add one to TInstructionRecord (mirror
    // of Destination/SourceA sentinels) rather than overloading the Value
    // field — #0 is a legitimate immediate.
  end;
  ...
  TInstructionRecord = record
    ...
    // Pseudo-instruction tagging.  PseudoKind=pkNone for normal
    // instructions and directives.  When pkBHI / pkBLS, BranchSize
    // carries the .S / .L hint and Operands[Length-1] is the user
    // target label.  When pkSHL_N / pkSHR_N / pkASR_N, Destination is
    // Dn and Immediate carries the count.
    IsPseudo:    Boolean;
    PseudoKind:  TPseudoKind;
    BranchSize:  TParsedBranchSize;

    // Listing decoration for pseudo expansions.  Set by
    // K16_Assembler.ExpandPseudoInstruction on records emitted as part
    // of a pseudo expansion; consumed by K16_Listing.
    //
    //   IsPseudoExpansion : True on every record produced by expansion
    //                       (tells the listing generator NOT to
    //                       deduplicate the disassembled-mnemonic
    //                       column for these records).
    //   PseudoBanner      : non-empty only on the FIRST record of an
    //                       expansion.  Text of the auto-generated
    //                       summary line, e.g.
    //                       "SHL D0, #6  →  SHL4 / SHL / SHL  (9c, 3w)".
    //                       The listing generator emits this as a
    //                       ';'-prefixed comment line *before* the
    //                       first bytecode line of the expansion.
    //   TrailingComment   : non-empty only on the FIRST record of an
    //                       expansion if the user's source line had a
    //                       trailing ';' comment.  Listing generator
    //                       appends it as a ';' comment on the first
    //                       bytecode line.
    IsPseudoExpansion: Boolean;
    PseudoBanner:      string;
  ...
  private
    ...
    // Pseudo-instruction recognition helpers (see TPseudoKind).
    // IsBHIorBLSMnemonic returns True for 'BHI', 'BLS', 'BHI.S',
    // 'BHI.L', 'BLS.S', 'BLS.L'.  ParseBranchSize extracts the .S/.L
    // hint.  IsShiftPseudoMnemonic returns True for the shift mnemonics
    // ONLY when a two-operand form is being parsed (caller decides).
    function  IsBHIorBLSMnemonic(const Mnemonic: string): Boolean;
    function  ParseBranchSize(const Mnemonic: string): TParsedBranchSize;
    function  GetBaseBHIBLSMnemonic(const Mnemonic: string): string;
    procedure ProcessBranchPseudo(var Instr: TInstructionRecord);
    procedure ProcessShiftPseudo(var Instr: TInstructionRecord;
                                 Kind: TPseudoKind);
...
begin
  ...
  // Look for + or - as the offset SEPARATOR. Pick whichever appears
  // first — that defines the split point. A minus that comes later
  // (e.g. inside the literal '#-4' after a '+' split, or as part of an
  // expression 'SYMBOL-4') is part of the offset, not the separator.
  PlusPos := Pos('+', Inner);
  MinusPos := Pos('-', Inner);

  if (PlusPos > 0) and ((MinusPos = 0) or (PlusPos < MinusPos)) then
  begin
    SplitPos := PlusPos;
    SplitIsMinus := False;
  end
  ...
  begin
    ...
    begin
      ...
      // Check if the offset is symbolic (contains a letter outside hex digits)
      // BEFORE peeling off any 'w' suffix — otherwise a symbol named e.g.
      // 'FOOw' or an expression 'FOO+2' loses its trailing letter and gets
      // misclassified.  'w' as a word-count multiplier is only valid on a
      ...
      // 'w' suffix only valid on numerics — strip and remember if so
      IsWordSuffix := False;
      if (not IsSymbolic) and (Length(OffsetPart) > 0) and
         (OffsetPart[Length(OffsetPart)] in ['w', 'W']) then
      begin
        IsWordSuffix := True;
      ...
      begin
        ...
        // For minus-split with symbolic offset, prepend '-' so the
        // resolver's expression evaluator negates the resolved value.
        // E.g. '[XY1-#FOO]' → OffsetSymbol = '-FOO' → resolver returns -FOO.
        if SplitIsMinus then
      ...
      // Apply negative sign if the SPLIT operator was '-' (e.g. '[XY1-#4]').
      // For numeric offsets, negate the parsed value.  For symbolic offsets,
      // negation is already encoded in OffsetSymbol via the leading '-'.
      // Note: a leading '-' inside the literal itself (e.g. '[XY1+#-4]') is
      // already captured by TryStrToInt above and must not be re-negated.
      if SplitIsMinus and (not Result.HasSymbolOffset) then
    ...
    begin
      ...
      begin
        ...
        begin
          Result.HasSymbolOffset := True;
          // Same negation handling as the '#'-prefixed path: prepend '-'
          // to the symbol expression so the resolver evaluates -SYMBOL.
          if SplitIsMinus then
            Result.OffsetSymbol := '-' + OffsetPart
          else
            Result.OffsetSymbol := OffsetPart;
...
// ---------------------------------------------------------------------
// Pseudo-instruction recognition (see TPseudoKind)
// ---------------------------------------------------------------------

function TK16Parser.IsBHIorBLSMnemonic(const Mnemonic: string): Boolean;
var
  Base: string;
begin
  Base := GetBaseBHIBLSMnemonic(Mnemonic);
  Result := SameText(Base, 'BHI') or SameText(Base, 'BLS');
end;

function TK16Parser.GetBaseBHIBLSMnemonic(const Mnemonic: string): string;
begin
  // Strip .S or .L suffix to get base mnemonic.  Shares logic with
  // GetBaseBranchMnemonic but kept separate so changes to native
  // branch handling don't ripple to pseudos.
  Result := Mnemonic;
  if Result.EndsWith('.S') or Result.EndsWith('.L') then
    Result := Copy(Result, 1, Length(Result) - 2);
end;

function TK16Parser.ParseBranchSize(const Mnemonic: string): TParsedBranchSize;
begin
  if Mnemonic.EndsWith('.S') then       Result := pbsShort
  else if Mnemonic.EndsWith('.L') then  Result := pbsLong
  else                                   Result := pbsAuto;
end;

procedure TK16Parser.ProcessBranchPseudo(var Instr: TInstructionRecord);
// Handle BHI / BLS (with optional .S / .L).
// Accepted forms:
//   BHI target            (one operand — target only)
//   BHI Dn, target        (two operands — Dn is documentation)
// Identical for BLS.  Sets IsPseudo / PseudoKind / BranchSize and
// leaves the target as the LAST operand string so the expander knows
// where to find it.
var
  Base: string;
begin
  Base := GetBaseBHIBLSMnemonic(Instr.Mnemonic);

  Instr.IsPseudo   := True;
  Instr.BranchSize := ParseBranchSize(Instr.Mnemonic);
  if SameText(Base, 'BHI') then
    Instr.PseudoKind := pkBHI
  else
    Instr.PseudoKind := pkBLS;

  // Operand-count validation
  if (Length(Instr.Operands) < 1) or (Length(Instr.Operands) > 2) then
  begin
    AddError(Format('%s requires 1 or 2 operands: %s [Dn,] target',
                    [Base, Base]));
    Exit;
  end;

  // Two-operand form: first operand should be a D register
  // (documentation — not used by expansion).  Soft-validate it.
  if Length(Instr.Operands) = 2 then
  begin
    Instr.Destination := TRegister.Parse(Instr.Operands[0]);
    if not Instr.Destination.IsValid or
       (Instr.Destination.RegType <> rtData) then
      AddWarning(Format('%s: first operand should be a D register (got %s)',
                        [Base, Instr.Operands[0]]));
  end;

  // Mode is a placeholder — pseudo expansion replaces this record with
  // native branch records, each of which gets its own mode set by the
  // parser when ParseLine is re-invoked on the emitted text.
  Instr.Mode := amRegReg;
end;

procedure TK16Parser.ProcessShiftPseudo(var Instr: TInstructionRecord;
                                        Kind: TPseudoKind);
// Handle the two-operand form SHL/SHR/ASR Dn, #count.
// Pre-condition: caller has already verified Operands has exactly 2
// elements and Operands[1] starts with '#'.
//
// Note: the parser DOES NOT decompose here.  It tags the record so
// FirstPass can call K16_PseudoShift.ExpandShift.  The one-operand
// form (e.g. SHL Dn) stays a native LOOKUP instruction and goes
// through the encoder path unchanged.
begin
  Instr.IsPseudo   := True;
  Instr.PseudoKind := Kind;
  ...
  if not Instr.Destination.IsValid or
     (Instr.Destination.RegType <> rtData) then
  begin
    AddError(Format('%s: destination must be a D register (D0-D3), got %s',
                    [Instr.Mnemonic, Instr.Operands[0]]));
    Exit;
  end;

  // Parse count immediate
  Instr.Immediate := TImmediateValue.Parse(Instr.Operands[1]);
  if Instr.Immediate.IsSymbol then
  begin
    AddError(Format('%s: count must be a numeric constant, not a symbol',
                    [Instr.Mnemonic]));
    Exit;
  end;
  if Instr.Immediate.IsSigned then
  begin
    AddError(Format('%s: negative shift count', [Instr.Mnemonic]));
    Exit;
  end;

  Instr.Mode := amImm5;  // Placeholder — expanded in FirstPass.
end;
...
begin
  ...
  // =========================================================================
  // Pseudo-instruction dispatch — fires BEFORE native instruction handlers.
  // Pseudos identified here are tagged with IsPseudo + PseudoKind and
  // expanded by FirstPass via K16_PseudoBranch / K16_PseudoShift.
  //
  //   BHI / BHI.S / BHI.L  target               (1 or 2 operands)
  //   BLS / BLS.S / BLS.L  target
  //   SHL Dn, #count       (two-operand form only — one-operand is native)
  //   SHR Dn, #count
  //   ASR Dn, #count
  // =========================================================================

  // BHI / BLS — always pseudos (no native equivalents).  Note: native
  // BHS and BLO already exist and route through IsBranchInstruction
  // unchanged.
  if IsBHIorBLSMnemonic(Result.Mnemonic) then
  begin
    ProcessBranchPseudo(Result);
    if not ValidateRegisterCombination(Result) then
      AddError(Format('Invalid register combination on line %d', [LineNumber]));
    Exit;
  end;

  // SHL / SHR / ASR — pseudo ONLY when called with two operands and
  // the second is an immediate (#count).  One-operand and register-
  // count forms continue to the existing LOOKUP encoder path below.
  if (Length(Result.Operands) = 2) and
     (Length(Result.Operands[1]) > 0) and
     (Result.Operands[1][1] = '#') then
  begin
    if SameText(Result.Mnemonic, 'SHL') then
    begin
      ProcessShiftPseudo(Result, pkSHL_N);
      Exit;
    end
    else if SameText(Result.Mnemonic, 'SHR') then
    begin
      ProcessShiftPseudo(Result, pkSHR_N);
      Exit;
    end
    else if SameText(Result.Mnemonic, 'ASR') then
    begin
      ProcessShiftPseudo(Result, pkASR_N);
      Exit;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if Result.MemoryRef.HasOffset or Result.MemoryRef.HasSymbolOffset
             or Result.MemoryRef.HasRegisterOffset then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result.MemoryRef := TMemoryRef.Parse(Result.Operands[1]);
        if Result.MemoryRef.HasOffset or Result.MemoryRef.HasSymbolOffset
             or Result.MemoryRef.HasRegisterOffset then
        begin
...
begin
  ...
  // Warn on negative immediate literals for arithmetic instructions only.
  // e.g. ADD D0, #-5 silently encodes $FFFB — use SUB for subtraction.
  // Bit-pattern ops (AND/OR/XOR/NOT/LOADI) are intentionally NOT warned:
  // '#-1' meaning $FFFF is an idiomatic mask / sign-extend pattern.
  // The assembler-level validation in EncodeInstruction enforces the
  // hard error for arithmetic ops; this warning is a softer earlier hint.
  if Instr.Immediate.IsSigned and
     (SameText(Instr.Mnemonic, 'ADD') or SameText(Instr.Mnemonic, 'ADC') or
      SameText(Instr.Mnemonic, 'SUB') or SameText(Instr.Mnemonic, 'SBC') or
      SameText(Instr.Mnemonic, 'CMP')) then
