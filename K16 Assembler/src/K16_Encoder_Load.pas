      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // Resolve symbol to absolute address. Pass with '#' prefix
              // so the resolver routes through its expression evaluator,
              // enabling forms like [PC+label+4].
        ...
        else if TempMemRef.HasOffset or TempMemRef.HasSymbolOffset then
        begin
          // Resolve offset value (literal or symbolic).
          // Pass the symbol with a '#' prefix so the resolver routes through
          // its expression evaluator — this enables [XY+#SYMBOL+2] etc.
          if TempMemRef.HasSymbolOffset then
            IntValue := Integer(SymbolResolver('#' + TempMemRef.OffsetSymbol, Instr.LineNumber))
          else
            IntValue := TempMemRef.Offset;

          // Check IMM5 range (0-31) - applies to both literal and symbolic
          if (IntValue < 0) or (IntValue > 31) then
          begin
            if TempMemRef.HasSymbolOffset then
              ErrorReporter(Format('Mode 11 [XY+#%s] offset out of range (0-31): symbol "%s" = %d',
                [TempMemRef.OffsetSymbol, TempMemRef.OffsetSymbol, IntValue]), Instr.LineNumber)
            else
