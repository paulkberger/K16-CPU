...
begin
  ...
  // Count 16: TWO ASR8s — produces sign-saturation ($FFFF for negative,
  // $0000 for positive) in 6 cycles / 2 words.  This is much better
  // than the spec's suggested 16-ASR chain (48c/16w) and produces
  // identical semantics: after two sign-fill shifts by 8, the register
  // contains 16 copies of the original sign bit.
  // Counts > 16 are also covered by ASR8 / ASR8 — once the value is
  // sign-saturated, further sign-fill shifts are no-ops.
  // Warning is still emitted suggesting SHR if zero-fill is intended.
  // -------------------------------------------------------------------
  ASR_TABLE[0]  := nil;
  ...
  ASR_TABLE[16] := TShiftRow.Create('ASR8', 'ASR8');
end;
...
begin
  ...
  // Optimisation hint for SHL/SHR at counts 14 and 15 ---------------------
  // These two counts have substantially cheaper expansions using the
  // mask-and-rotate trick, but those alternatives clobber C/Z/N (AND and
  // related logical ops are NOT flag-preserving, unlike LOOKUP shifts).
  //
  // We don't AUTO-use the alternative because the pseudo's documented
  // contract is "flags not affected" (mirrors LOOKUP-shift semantics).
  // Instead, hint at the existence of the alternative so the user can
  // hand-code it where flag preservation isn't required.
  //
  //   SHL #14 :  AND #3 / ROR Dn / ROR Dn        (9c, 3w  vs 15c, 5w)
  //   SHL #15 :  AND #1 / ROR Dn                 (6c, 2w  vs 18c, 6w)
  //   SHR #14 :  ROL Dn / ROL Dn / AND #3        (9c, 3w  vs 12c, 4w)
  //   SHR #15 :  ROL Dn / AND #1                 (6c, 2w  vs 15c, 5w)
  //
  // ASR has no corresponding shortcut at these counts — the sign-fill
  // semantics can't be reproduced with a simple mask-and-rotate.
  if Result.Warning = '' then
  begin
    case Direction of
      sdSHL:
        if Effective = 14 then
          Result.Warning := Format(
            'SHL %s, #14 could be ''AND %s, #3 / ROR / ROR'' (9c vs 15c, 3w vs 5w) if flags don''t matter',
            [RegName, RegName])
        else if Effective = 15 then
          Result.Warning := Format(
            'SHL %s, #15 could be ''AND %s, #1 / ROR'' (6c vs 18c, 2w vs 6w) if flags don''t matter',
            [RegName, RegName]);
      sdSHR:
        if Effective = 14 then
          Result.Warning := Format(
            'SHR %s, #14 could be ''ROL / ROL / AND %s, #3'' (9c vs 12c, 3w vs 4w) if flags don''t matter',
            [RegName, RegName])
        else if Effective = 15 then
          Result.Warning := Format(
            'SHR %s, #15 could be ''ROL / AND %s, #1'' (6c vs 15c, 2w vs 5w) if flags don''t matter',
            [RegName, RegName]);
    end;
  end;
