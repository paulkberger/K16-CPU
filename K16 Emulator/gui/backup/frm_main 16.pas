...
const
  CONTEXT = 4;   { instructions to show above PC }
var
  addr : TAddr;
  row  : Integer;
  used : Integer;
...
begin
  ...
  { Walk back CONTEXT instructions before PC to get context above }
  { Simple approach: step back by 2 bytes at a time (min instruction size) }
  addr := (CPU.PC - TAddr(CONTEXT * 2)) and ADDR_MASK;
...
const
  CONTEXT = 4;   { must match RefreshDisasm }
var
  grid : TStringGrid;
  txt  : string;
begin
