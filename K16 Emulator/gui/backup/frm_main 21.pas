...
begin
  ...
  begin
    ...
    if addr = CPU.PC then
      ...
      StringGridDisasm.Cells[0, row] := '';
    disline := Disassemble(addr, used);
    { Split: first token is address ($XXXXXX), rest is instruction }
    sppos := Pos('  ', disline);
    if sppos > 0 then
    begin
      StringGridDisasm.Cells[1, row] := Trim(Copy(disline, 1, sppos));
      StringGridDisasm.Cells[2, row] := Trim(Copy(disline, sppos+1, MaxInt));
    end else
    begin
      StringGridDisasm.Cells[1, row] := disline;
