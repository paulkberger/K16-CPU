...
{ Rebuild FVideoBmp from Mem[] at native resolution. }
var
  y, x, bit : Integer;
  sl         : PByte;
  b          : Byte;
...
begin
  ...
  MemoTerminal.Lines.Add(Format('Paint bmp=%dx%d', [FVideoBmp.Width, FVideoBmp.Height]));
  { Simple aspect-fit: scale to fit panel }
  scale := Min(W / FVideoBmp.Width, H / FVideoBmp.Height);
  rw    := Round(FVideoBmp.Width  * scale);
  rh    := Round(FVideoBmp.Height * scale);
  rx    := (W - rw) div 2;
  ry    := (H - rh) div 2;
  pb.Canvas.Draw(rx, ry, FVideoBmp);
end;
