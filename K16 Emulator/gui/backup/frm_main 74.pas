...
{ Sine rainbow, period=512 entries.
  Adjacent entries differ by ~1.5 RGB units.
  Plasma sum range 0..214 → palette 0..214 → 150° hue arc:
    lime-green (peak) → yellow → amber → red → magenta (trough).
  Rate at max sine slope: 1.3 steps/px × 1.5 RGB/step ≈ 2 RGB/px → invisible. }
var
  i: Integer;
  phase: Double;
begin
  for i := 0 to 255 do
  begin
    phase := i / 160.0 * 2 * Pi;       { period = 160 = 3-sine SHR range → cyclic: [0]=[160] }
    FSmoothPal[i] :=
      (LongWord(Round(128 + 127 * Sin(phase)))           shl 16) or
      (LongWord(Round(128 + 127 * Sin(phase + 2*Pi/3)))  shl  8) or
       LongWord(Round(128 + 127 * Sin(phase + 4*Pi/3)));
  end;
end;
