...
begin
  ...
  FTerminal.Parent   := GroupBoxTerminal;   { same parent as MemoTerminal }
  FTerminal.Align    := alClient;
  FTerminal.OnKey    := TerminalKey;
  FTerminal.OnPaste  := TerminalPaste;
  MemoTerminal.Visible := False;     { hide old memo }
...
begin
  ...
  begin
    ...
    pbVideo.Width  := 0;
    pbVideo.Height := 0;
    GroupBox2.Height := 62;   { caption only }
    pbVideo.Invalidate;
    Exit;
  end;
  pbVideo.Width    := OUT_W[mode];
  pbVideo.Height   := OUT_H[mode];
  GroupBox2.Height := OUT_H[mode] + 46;   { pbVideo.Top(16) + height + 30 caption }
...
begin
  { Direct write to VT100 terminal — no queue needed, Invalidate is deferred }
  if Assigned(FTerminal) then
    FTerminal.WriteChar(ch);
end;
