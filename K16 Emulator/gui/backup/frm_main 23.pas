...
begin
  ...
  if FCPUThread.Stepping then Exit;   { not running }

  nowTick := GetTickCount64;
  elapsed := nowTick - FLastTick;
  if elapsed < 500 then Exit;         { sample every 500ms }

  cycles := CPU.CycleCount;
  if (FLastTick > 0) and (elapsed > 0) then
  begin
    mhz := (cycles - FLastCycles) / elapsed / 1000.0;
    ...
    if mhz >= 1.0 then
      LabelStatus.Caption := Format('Running [%s]  %.2f MHz', [modeStr, mhz])
    else if mhz >= 0.001 then
      LabelStatus.Caption := Format('Running [%s]  %.0f KHz', [modeStr, mhz * 1000])
    else
      LabelStatus.Caption := Format('Running [%s]  %.0f Hz',  [modeStr, mhz * 1000000]);
  end;
