...
begin
  ...
  RefreshMem;
  DrainTermQueue;
  if FVideoMode > 0 then PbVideoPaint(pbVideo);
end;
...
begin
  ...
  RefreshRegs;
  RefreshDisasm;
  PbVideoPaint(pbVideo);   { always repaint on halt — FVideoMode checked inside }
...
begin
  DrainTermQueue;
  UpdateSpeedLabel;
  if (FVideoMode > 0) and (FCPUThread <> nil) and not CPU.Halted then
    PbVideoPaint(pbVideo);
...
begin
  FVideoMode := mode;
  { Note: called from CPU thread via IOWriteHook — do NOT touch UI here }
end;
