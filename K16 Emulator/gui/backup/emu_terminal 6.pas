...
{ Scroll visible rows [top..bot] up by 1; bottom row blanked.
  When top=0, the discarded top row is pushed into scrollback by advancing FBufTop.
  Invariant: FBufTop + FRows <= FBufLines always. }
var r: Integer;
begin
  if top = 0 then
  begin
    if FBufTop + FRows < FBufLines then
    begin
      { Scrollback has room — just advance FBufTop }
      Inc(FBufTop);
    end
    else
    begin
      { Scrollback full — shift entire buffer up one.
        Content moves from index N to N-1, so FBufTop also decrements
        to keep pointing at the same logical top-of-visible row. }
      for r := 1 to FBufLines - 1 do
        FBuffer[r-1] := FBuffer[r];
      if FBufTop > 0 then Dec(FBufTop);
    end;
    { Blank the new bottom visible row }
    BlankRow(FBufTop + bot);
  end
  else
  begin
    { Scroll within region only — no scrollback involvement }
    for r := top to bot - 1 do
      FBuffer[FBufTop + r] := FBuffer[FBufTop + r + 1];
    BlankRow(FBufTop + bot);
  end;
  SetDirty;
end;
...
var
  ...
  r, c     : Integer;
begin
  newLines := SCROLLBACK_LINES + newRows + 1;  { +1 spare so FBufTop+FRows < FBufLines always }
...
begin
  ...
  FSB.SmallChange:= 1;
  { When FScrollPos=0 (live view), position thumb at avail so it sits at bottom }
  FSB.Position   := Max(0, Min(avail, avail - FScrollPos));
...
begin
  ...
  { viewTop: buffer row index of top visible line.
    Clamp FBufTop so we never read past end of buffer. }
  if FBufTop + FRows > FBufLines then
    FBufTop := FBufLines - FRows;
        ...
        32..126, 128..255:
          begin
            ...
            if (FCurX >= 0) and (FCurX < FCols) and
               (FCurY >= 0) and (FCurY < FRows) and
               (FBufTop + FCurY < FBufLines) then
              FBuffer[FBufTop + FCurY][FCurX] := cell;
            Advance;
          end;
      end;
      ...
      begin
        case P(0,0) of
          0: begin BlankRange(FCurY, FCurX, FCols-1);
                   for r := FCurY+1 to FRows-1 do BlankRow(FBufTop + r); end;
          1: begin BlankRange(FCurY, 0, FCurX);
                   for r := 0 to FCurY-1 do BlankRow(FBufTop + r); end;
          2: begin for r := 0 to FRows-1 do BlankRow(FBufTop + r);
                   FCurX := 0; FCurY := 0; end;
        end;
      end;
    'K':
    ...
    'P':
      begin
        ...
        for c := FCurX to FCols-1-n do
          FBuffer[FBufTop + FCurY][c] := FBuffer[FBufTop + FCurY][c+n];
