...
begin
  ...
  pbVideo.Width    := 0;
  pbVideo.Height   := 0;
  GroupBoxVideo.Height := 62;   { caption only — SetVideoMode resizes on first write }
...
begin
  ...
  if CheckBoxLiveUpdate.Checked then
  begin
    RefreshRegs;
    RefreshDisasm;
    RefreshMem;
...
begin
  ...
  begin
    ...
    pbVideo.Width  := 0;
    pbVideo.Height := 0;
    GroupBoxVideo.Height := 62;   { caption only }
    pbVideo.Invalidate;
    Exit;
  end;
  pbVideo.Width    := OUT_W[mode];
  pbVideo.Height   := OUT_H[mode];
  GroupBoxVideo.Height := OUT_H[mode] + 46;   { pbVideo.Top(16) + height + 30 caption }
