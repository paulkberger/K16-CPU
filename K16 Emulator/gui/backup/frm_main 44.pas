...
begin
  ...
  FVideoMode:=0; VideoMode:=0; FrameDirty:=False;
  pbVideo.Invalidate;
  if FileName <> '' then
  begin
    try
...
begin
  ...
  RefreshMem;
  DrainTermQueue;
  if FrameDirty then begin FrameDirty:=False; FVideoMode:=VideoMode; if FVideoMode>0 then PbVideoPaint(pbVideo); end;
end;
...
begin
  ...
  RefreshRegs;
  RefreshDisasm;
  FrameDirty:=False; FVideoMode:=VideoMode; if FVideoMode>0 then PbVideoPaint(pbVideo);
