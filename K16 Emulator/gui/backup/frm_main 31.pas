...
begin
  ...
  if elapsedSec < 0.2 then Exit;    { sample every 200ms }
  ...
  begin
    ...
    { Debug: show raw values when measured speed looks wrong }
    if (mhz < 0.1) and (TargetMHz >= 1) then
      MemoTerminal.Lines.Add(Format('[SPD] elapsed=%.3f cyc=%d last=%d delta=%d',
        [elapsedSec, cycles, FLastCycles, Int64(cycles)-Int64(FLastCycles)]));

    case ComboBox1.ItemIndex of
      1: LabelSpeed.Caption := Format('%d MHz  (%.2f)', [TargetMHz, mhz]);
    else
