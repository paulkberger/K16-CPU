...
type
  ...
  private
    ...
    FBinFile    : string;
    FVideoBmp   : TBitmap;
    FVideoW, FVideoH, FVideoRW, FVideoRH, FVideoRX, FVideoRY : Integer;
    FVideoScale : Double;
    FVideoMode  : Word;
    FMemBase    : TAddr;
  ...
  begin
    try
      ...
      begin
        ...
        CPU.PC   := LoadAddr;   { start at the loaded address }
...
begin
  ...
  RefreshMem;
  DrainTermQueue;
  if VideoMode > 0 then begin UpdateVideoBmp; pbVideo.Repaint; end;
end;
...
begin
  ...
  begin
    ...
    begin
      FVideoW := pbVideo.Width;
      FVideoH := pbVideo.Height;
      FVideoScale := Min(FVideoW / FVideoBmp.Width, FVideoH / FVideoBmp.Height);
      FVideoRW := Round(FVideoBmp.Width  * FVideoScale);
      FVideoRH := Round(FVideoBmp.Height * FVideoScale);
      FVideoRX := (FVideoW - FVideoRW) div 2;
      FVideoRY := (FVideoH - FVideoRH) div 2;
      pbVideo.Canvas.CopyRect(Rect(FVideoRX, FVideoRY, FVideoRX+FVideoRW, FVideoRY+FVideoRH), FVideoBmp.Canvas, Rect(0, 0, FVideoBmp.Width, FVideoBmp.Height));
    end;
  end;
  ...
  begin
    ...
    speedMhz   := cyclesDone;
    speedMhz   := speedMhz / haltSec;
    speedMhz   := speedMhz / 1000000.0;
    haltMhz    := speedMhz;
    if haltMhz >= 1.0 then
      LabelSpeed.Caption := Format('%.2f MHz', [haltMhz])
    else
      LabelSpeed.Caption := Format('%.2f KHz', [haltMhz * 1000]);
  end
end;
...
begin
  DrainTermQueue;
  UpdateSpeedLabel;
  if (VideoMode > 0) and (FCPUThread <> nil) and not CPU.Halted then
  begin
    UpdateVideoBmp;
    if (FVideoBmp.Width > 0) and (FVideoBmp.Height > 0) then
    begin
...
{ Rebuild FVideoBmp from Mem[]. Call from main thread only. }
var
  y, x, bit : Integer;
  sl         : PByte;
  b          : Byte;
...
begin
  if VideoMode = 1 then
  begin
    if FVideoBmp.Height <> 768 then
    begin
      FVideoBmp.PixelFormat := pf32bit;
      FVideoBmp.SetSize(1024, 768);
    end;
    for y := 0 to 767 do
    begin
  ...
  begin
    if FVideoBmp.Height <> 480 then
    begin
      FVideoBmp.PixelFormat := pf32bit;
      FVideoBmp.SetSize(640, 480);
    end;
    for y := 0 to 479 do
    begin
      sl := FVideoBmp.ScanLine[y];
...
begin
  ...
  pb.Canvas.FillRect(Rect(0,0,W,H));
  UpdateVideoBmp;   { ensure FVideoBmp is current before drawing }
  ...
  pb.Canvas.CopyRect(dest, FVideoBmp.Canvas, Rect(0, 0, FVideoBmp.Width, FVideoBmp.Height));
end;
