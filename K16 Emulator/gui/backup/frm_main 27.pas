...
begin
  ...
  MemoTerminal.Lines.Add(Format('DEBUG: start=%g now=%g sec=%g cycles=%d startcyc=%d',
    [FRunStartTime, Now, haltSec, CPU.CycleCount, FRunStartCycles]));
  ...
  begin
    ...
    haltMhz    := speedMhz;
    MemoTerminal.Lines.Add(Format('DEBUG2: mhz=%g rawcyc=%d haltsec=%g', [haltMhz, cyclesDone, haltSec]));
    if haltMhz >= 1.0 then
    begin
      LabelSpeed.Caption := Format('%.1f MHz', [haltMhz]);
      LabelStatus.Caption := LabelStatus.Caption + '  ' + LabelSpeed.Caption;
    end
    else if haltMhz >= 0.001 then
    begin
      LabelSpeed.Caption := Format('%.1f KHz', [haltMhz * 1000]);
      LabelStatus.Caption := LabelStatus.Caption + '  ' + LabelSpeed.Caption;
    end
    else
    begin
      LabelSpeed.Caption := Format('%.0f Hz', [haltMhz * 1000000]);
      LabelStatus.Caption := LabelStatus.Caption + '  ' + LabelSpeed.Caption;
    end;
  end
  else
    MemoTerminal.Lines.Add(Format('DEBUG2: skipped start=%g sec=%g', [FRunStartTime, haltSec]));
end;
...
begin
  ...
  if elapsedSec < 0.5 then Exit;    { sample every 500ms }
  ...
  begin
    mhz := (Double(cycles) - Double(FLastCycles)) / elapsedSec / 1000000.0;
    if mhz >= 1.0 then
      LabelSpeed.Caption := Format('%s  %.0f MHz', [modeStr, mhz])
    else if mhz >= 0.001 then
      LabelSpeed.Caption := Format('%s  %.0f KHz', [modeStr, mhz * 1000])
    else
      LabelSpeed.Caption := Format('%s  %.0f Hz',  [modeStr, mhz * 1000000]);
  end;
...
var
  ...
  disline  : string;
  sppos    : Integer;
  histAddrs: array[0..15] of TAddr;  { addresses for rows above PC }
  h        : Integer;
...
begin
  ...
  begin
    ...
    if histAddrs[h] = 0 then histAddrs[h] := CPU.PC;
  end;

  { Fill rows above PC from history }
  for row := 0 to CONTEXT - 1 do
    PutRow(row, histAddrs[row], False);
