...
begin
  ...
  { Disasm grid: arrow col + address col + disasm text col }
  ...
  begin
    try
      ...
      begin
        ...
        CPU.PC   := LoadAddr;
        LabelStatus.Caption := ExtractFileName(FileName) +
          Format('  —  Loaded at $%6.6X', [LoadAddr]);
      end
      else
      begin
...
begin
  ...
  if FPCHistory[LastIdx] = CPU.PC then Exit;
...
begin
  { Called from VideoModeHook on the main thread via Synchronize.
    The global VideoMode in emu_mem is already set by MemWriteWord;
...
{ Produces a 1024x768 bitmap that exactly matches the fixed TImage size.
  Mode 1 (1bpp 1024x768): direct 1:1 copy — no scaling.
  Mode 2 (8bpp 640x480):  nearest-neighbour x1.6 upscale — both modes
    are 4:3 so 640x480 fills 1024x768 perfectly with no letterboxing.
  FVideoBmp is reused across calls; resized only when needed.
  Assign() into Picture.Bitmap forces a proper repaint — TImage.Stretch
  is never exercised, so the thin-strip StretchBlt issue cannot occur. }
const
  OUT_W = 1024;
  OUT_H = 768;
var
...
begin
  ...
  if (FVideoBmp.Width <> OUT_W) or (FVideoBmp.Height <> OUT_H) then
  begin
    FVideoBmp.PixelFormat := pf32bit;
    FVideoBmp.SetSize(OUT_W, OUT_H);
  end;

  if VideoMode = 1 then
  begin
    { 1bpp mono — 128 bytes per row, MSB first }
  ...
  else  { VideoMode = 2 — 8bpp 640x480, nearest-neighbour scale to 1024x768 }
  begin
    for y := 0 to OUT_H - 1 do
    begin
      sy := y * 480 div OUT_H;      { source row: 0..479 }
      sl := FVideoBmp.ScanLine[y];
      for x := 0 to OUT_W - 1 do
      begin
        sx := x * 640 div OUT_W;   { source col: 0..639 }
  ...
  ImageVideo.Picture.Bitmap.Assign(FVideoBmp);
end;

procedure TfrmMain.PbVideoPaint(Sender: TObject);
begin
  { Orphaned — no TPaintBox on the form. Kept to satisfy the class declaration. }
end;
