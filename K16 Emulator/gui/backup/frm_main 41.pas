...
begin
  ...
  begin
    pbVideo.Invalidate;
    pbVideo.Update;     { force synchronous repaint — don't wait for message queue }
  end;
  if FVideoMode > 0 then pbVideo.Invalidate;   { refresh video on halt }
...
begin
  FVideoMode := mode;
  { Do NOT call pbVideo.Invalidate here — this is called from the CPU thread.
    Repaints are handled by TmrRefresh and OnCPUHalt on the main thread. }
end;
