...
type
  ...
  private
    ...
    function  BufRow(visRow: Integer): Integer; inline;
...
begin
  ...
  Canvas.Font.Style   := [];
  FCellW := Canvas.TextWidth('W');
  FCellH := Canvas.TextHeight('W');
...
begin
  ...
  if FScrollPos = 0 then
    ...
    FSB.Position := Max(0, avail - FScrollPos);
end;
  ...
  case FState of
    ...
    psNormal:
      case ch of
        ...
        9:  DoTab;
        10: DoLF;
        13: begin DoCR; DoLF; end;   { CR treated as CR+LF for dumb terminal mode }
        27: FState := psEsc;
        32..126, 128..255:
          begin
            cell.Ch   := Chr(ch);
...
procedure TK16Terminal.DoMouseDown(Sender: TObject; Button: TMouseButton;
                                   Shift: TShiftState; X, Y: Integer);
begin
  SetFocus;
end;

procedure TK16Terminal.DoKeyPress(Sender: TObject; var Key: Char);
begin
  if Ord(Key) >= 32 then
  begin
...
procedure TK16Terminal.DoKeyDown(Sender: TObject; var Key: Word;
                                  Shift: TShiftState);
begin
  case Key of
