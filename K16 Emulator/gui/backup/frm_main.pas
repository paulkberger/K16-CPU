...
begin
  ...
  { Verbose disk logging follows Live Updates: when the user has
    Live Updates on, they want full per-IO trace in the Messages panel.
    With Live Updates off, keep disk logs quiet. }
  emu_disk.DiskLogVerbose := CheckBoxCPUUpdates.Checked;
  DiskInit;
...
begin
  ...
  { Live Updates on  → enable verbose disk logging (per-IO trace).
    Live Updates off → silence verbose disk logging. }
  emu_disk.DiskLogVerbose := CheckBoxCPUUpdates.Checked;
end;
...
begin
  ...
  try
    { Defaults: green, fast, live ticked }
    ...
    CheckBoxCPUUpdates.Checked := ini.ReadBool   ('UI', 'LiveUpdate', True);
...
begin
  ...
  try
    ...
    ini.WriteBool   ('UI', 'LiveUpdate', CheckBoxCPUUpdates.Checked);
    ini.UpdateFile;
  finally
    ini.Free;
  end;
