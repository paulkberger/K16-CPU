...
begin
  ...
  StringGridDisasm.ColWidths[1] := StringGridDisasm.Width - 50;
  ...
  { Memory grid: addr col + 16 byte columns }
  StringGridMemory.ColCount     := 17;   { 1 addr + 16 bytes }
  StringGridMemory.FixedCols    := 1;
  StringGridMemory.FixedRows    := 0;
  StringGridMemory.ColWidths[0] := 90;
  { byte columns -- narrow }
  StringGridMemory.ColWidths[1] := 30;
  StringGridMemory.ColWidths[2] := 30;
  StringGridMemory.ColWidths[3] := 30;
  StringGridMemory.ColWidths[4] := 30;
  StringGridMemory.ColWidths[5] := 30;
  StringGridMemory.ColWidths[6] := 30;
  StringGridMemory.ColWidths[7] := 30;
  StringGridMemory.ColWidths[8] := 30;
  StringGridMemory.ColWidths[9] := 30;
  StringGridMemory.ColWidths[10] := 30;
  StringGridMemory.ColWidths[11] := 30;
  StringGridMemory.ColWidths[12] := 30;
  StringGridMemory.ColWidths[13] := 30;
  StringGridMemory.ColWidths[14] := 30;
  StringGridMemory.ColWidths[15] := 30;
  StringGridMemory.ColWidths[16] := 30;
...
const
  CONTEXT = 3;   { instructions of history to show above PC }
var
  addr     : TAddr;
  row      : Integer;
  used     : Integer;
  histIdx  : Integer;
begin
  { Find a good start address CONTEXT steps back in history }
  histIdx := (FPCHistIdx - CONTEXT - 1) and 63;
  addr    := FPCHistory[histIdx];

  { Fall back to PC if history not yet populated }
  if addr = 0 then addr := CPU.PC;

  for row := 0 to StringGridDisasm.RowCount - 1 do
  begin
    { Mark current PC row with arrow }
    if addr = CPU.PC then
  ...
  begin
    ...
    for col := 1 to 16 do
      StringGridMemory.Cells[col, row] :=
        Format('%2.2X', [Mem[(addr + TAddr(col-1)) and ADDR_MASK]]);
  end;
end;
...
begin
  ...
  if txt <> '' then
    grid.Canvas.TextOut(aRect.Left + 2, aRect.Top + 2, txt);
end;
