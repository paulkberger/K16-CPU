...
type
  ...
  private
    ...
    FSmoothPal  : array[0..255] of LongWord;  { precomputed sine rainbow for VideoMode 3 }
...
begin
  ...
  { Precompute smooth sine rainbow palette for VideoMode 3 (plasma mode)
    Index i → phase i/256 * 2π → smooth RGB, ~3 units per step, no jumps }
  BuildSmoothPalette;
  FMemBase   := RESET_VEC;
  FBinFile   := '';
  FTermHead  := 0;
...
{ Glow ramp for product plasma.
  palette[0]=black, palette[127]=peak colour, palette[254]=black.
  Product plasma formula gives 0 at sine zero-crossings → grid lines are
  black/invisible; blobs (product near peak) glow bright.
  Uses a sin² brightness envelope with slow hue shift (fire-like: black→
  red→orange→yellow→white→yellow→orange→red→black). }
var
  i: Integer;
  t, brightness: Double;
  r, g, b: Integer;
begin
  ...
  begin
    t := i / 254.0;                         { 0..1 }
    brightness := Sin(t * Pi);              { 0→1→0 bump, peaks at i=127 }
    { Hue: black→red→orange→yellow at t=0.5, back to black }
    r := Round(brightness * 255);
    g := Round(brightness * brightness * 180);   { green lags behind = fire }
    b := Round(brightness * brightness * brightness * 60);  { tiny blue = warmth }
    FSmoothPal[i] :=
...
var r, g, bv: Integer;
begin
  { VideoMode 3: smooth sine rainbow — all 256 indices map to smooth gradient }
  if VideoMode = 3 then
  begin
