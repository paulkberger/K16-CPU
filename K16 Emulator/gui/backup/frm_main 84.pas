...
begin
  ...
  { Immediate terminal test — visible before CPU starts }
  DebugTermWrite(Format('[RUN BE=%d BA=$%6.6X]'#10,
                        [Ord(BreakEnabled), BreakAddr]));
...
begin
  ...
  try
    ...
    begin
      ...
      RefreshDisasm;
      RefreshMem;
      { Breakpoint hit — update status label once }
      if FCPUThread.BreakpointHit then
      begin
        FCPUThread.ClearBreakpointHit;
        LabelStatus.Caption := Format('Breakpoint $%6.6X', [CPU.PC]);
        LabelSpeed.Caption  := '';
      end;
    end;
  finally
