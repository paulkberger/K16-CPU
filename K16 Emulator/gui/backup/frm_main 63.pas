...
{ Writes pixels to FVideoBmp then blits directly to the panel via
  GetDC/BitBlt/ReleaseDC — bypasses LCL Canvas/BeginPaint which clips
  to the invalid region (empty after first WM_PAINT) causing blank updates.
  PbVideoPaint handles window-uncover redraws via the normal WM_PAINT path. }
const
  OUT_W = 1024;
  OUT_H = 768;
var
...
begin
  ...
  else  { VideoMode = 2 — nearest-neighbour 640x480 -> 1024x768 }
  begin
    for y := 0 to OUT_H - 1 do
    begin
      sy := y * 480 div OUT_H;
...
const
  SRCCOPY_ROP = $00CC0020;
begin
  if (FVideoBmp.Width > 0) and (FVideoBmp.Height > 0) then
    LCLIntf.BitBlt(Canvas.Handle, 0, 0,
                   FVideoBmp.Width, FVideoBmp.Height,
                   FVideoBmp.Canvas.Handle, 0, 0, SRCCOPY_ROP)
  else
  begin
