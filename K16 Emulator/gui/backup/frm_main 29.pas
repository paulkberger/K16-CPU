...
begin
  ...
  begin
    ...
    case ComboBox1.ItemIndex of
      1: { MHz mode -- show target, measured in parens }
         LabelSpeed.Caption := Format('MHz %d  (%.2f)', [TargetMHz, mhz]);
    else { Fast -- show measured }
      if mhz >= 1.0 then
        LabelSpeed.Caption := Format('Fast  %.2f MHz', [mhz])
      else
        LabelSpeed.Caption := Format('Fast  %.2f KHz', [mhz * 1000]);
    end;

    { DEBUG }
    MemoTerminal.Lines.Add(Format('[SPD] elapsed=%.3fs delta_cyc=%d mhz=%.3f',
      [elapsedSec, Int64(cycles) - Int64(FLastCycles), mhz]));
  end;
