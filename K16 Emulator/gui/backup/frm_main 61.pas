...
{ Write pixels into FVideoBmp offscreen buffer, then Invalidate pbVideo.
  OnPaint does Canvas.Draw(0,0,FVideoBmp) — proven to work.
  Mode 1: 1bpp mono 1024x768 direct copy.
  Mode 2: 8bpp 640x480, nearest-neighbour x1.6 to 1024x768 (exact 4:3). }
const
  OUT_W = 1024;
  OUT_H = 768;
var
...
begin
  if (FVideoBmp.Width > 0) and (FVideoBmp.Height > 0) then
    pbVideo.Canvas.Draw(0, 0, FVideoBmp)
  else
  begin
    pbVideo.Canvas.Brush.Color := clBlack;
    pbVideo.Canvas.FillRect(pbVideo.ClientRect);
  end;
end;
