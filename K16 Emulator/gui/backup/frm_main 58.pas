...
var
  ...
  c          : LongWord;
begin
  bmp := ImageVideo.Picture.Bitmap;
  if VideoMode = 1 then
  begin
    if (bmp.Width <> 1024) or (bmp.Height <> 768) then
    begin
      bmp.PixelFormat := pf32bit;
      bmp.SetSize(1024, 768);
    end;
    for y := 0 to 767 do
    begin
      sl := bmp.ScanLine[767 - y];
      for x := 0 to 127 do
      begin
  ...
  begin
    if (bmp.Width <> 640) or (bmp.Height <> 480) then
    begin
      bmp.PixelFormat := pf32bit;
      bmp.SetSize(640, 480);
    end;
