...
begin
  ...
  begin
    ...
    begin
      { Scrollback full — shift entire buffer up one, FBufTop stays }
      for r := 1 to FBufLines - 1 do
        FBuffer[r-1] := FBuffer[r];
      { FBufTop unchanged; last row is now at FBufTop + FRows - 1 }
    end;
...
begin
  ...
  { viewTop: buffer row index of the top visible screen line.
    FScrollPos=0 → live view starting at FBufTop.
    FScrollPos=N → scrolled back N lines, so start N rows before FBufTop. }
  viewTop := FBufTop - FScrollPos;
  if viewTop < SCROLLBACK_LINES then viewTop := SCROLLBACK_LINES;
