...
begin
  ...
  { DEBUG: scan raw 640x480 framebuffer before rendering }
  nonZero  := 0;
  firstRow := -1;
  lastRow  := -1;
  for y := 0 to 479 do
    for x := 0 to 639 do
      if Mem[FB_BASE + y * 640 + x] <> 0 then
      begin
        Inc(nonZero);
        if firstRow < 0 then firstRow := y;
        lastRow := y;
      end;
  MemoTerminal.Lines.Add(Format('VM=%d nonZero=%d rows=%d..%d Mem[0]=$%2.2X',
    [VideoMode, nonZero, firstRow, lastRow, Mem[FB_BASE]]));
