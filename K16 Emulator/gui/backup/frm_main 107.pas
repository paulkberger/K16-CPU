...
begin
  ...
  FillChar(FPCHistory, SizeOf(FPCHistory), 0);
  FPCHistIdx := 0;
  { Clear halted state so Run works after HALT or max-cycles stop.
    PC is left where it is; if the next instruction is another HALT
    we'll just halt again immediately, which is fine. }
  CPU.Halted   := False;
  CPU.HaltCode := 0;
...
begin
  if FBinFile = '' then begin ShowMessage('Load a .bin file first.'); Exit; end;
  { Allow stepping past a HALT -- clears the flag so the CPU thread loop
    won't immediately Break.  PC is unchanged; user can Reset if desired. }
  CPU.Halted   := False;
  CPU.HaltCode := 0;
  StartCPU;
  FCPUThread.DoStop;
