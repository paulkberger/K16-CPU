...
begin
  ...
  FVideoMode := 0;
  VideoMode  := 0;
  FVideoBmp.SetSize(0, 0);  { clear stale bitmap — forces full rebuild next paint }
  pbVideo.Invalidate;
  if FileName <> '' then
  begin
    try
...
begin
  ...
  RefreshMem;
  DrainTermQueue;
  if VideoMode > 0 then PbVideoPaint(pbVideo);
end;
...
begin
  ...
  RefreshRegs;
  RefreshDisasm;
  PbVideoPaint(pbVideo);   { build FVideoBmp from Mem[] }
  pbVideo.Repaint;         { force OS paint event to actually display it }
