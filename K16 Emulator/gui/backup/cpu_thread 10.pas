...
begin
  ...
  repeat
    ...
    { Breakpoint check — before Fetch so execution stops ON the breakpoint
      ...
      resume to prevent immediate re-trigger at the same PC. }
    if BreakEnabled then
    begin
      if (not FJustHitBreakpoint) and (CPU.PC = BreakAddr) then
      begin
        FStepping          := True;
        FJustHitBreakpoint := True;
        FBreakpointHit     := True;
        if Assigned(DebugOut) then
          DebugOut(Format(#10'[BP HIT $%6.6X]'#10, [CPU.PC]));
        Continue;
      end;
    end
    else
    begin
      { Fires every 10000 instructions so we can see if thread is running }
      if (BatchCount mod 10000 = 0) and Assigned(DebugOut) then
        DebugOut(Format('[PC=$%6.6X BE=%d BA=$%6.6X]'#10,
                        [CPU.PC, Ord(BreakEnabled), BreakAddr]));
    end;
