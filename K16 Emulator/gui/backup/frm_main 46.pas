...
begin
  ...
  FVideoMode := 0;
  FVideoBmp.Width := 0;   { clear stale image }
  pbVideo.Invalidate;
  if FileName <> '' then
  begin
    try
...
begin
  ...
  begin
    BuildVideoBmp;       { fill FVideoBmp from Mem[] }
    pbVideo.Invalidate;  { OS will call PbVideoPaint which StretchDraws it }
  end;
...
begin
  DrainTermQueue;
  UpdateSpeedLabel;
  FVideoMode := VideoMode;   { pick up mode set by CPU thread }
  if FVideoMode > 0 then pbVideo.Invalidate;
...
procedure TfrmMain.BuildVideoBmp;
{ Fill FVideoBmp from Mem[] according to FVideoMode.
  Can be called from any context — does NOT touch Canvas. }
var
  y, x, bit  : Integer;
  sl          : PByte;
  b           : Byte;
