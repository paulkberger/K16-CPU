...
begin
  ...
  pbVideo.Width    := 0;
  pbVideo.Height   := 0;
  GroupBox2.Height := 62;   { caption only — SetVideoMode resizes on first write }
...
begin
  ...
  { Collapse BS+space+BS (terminal erase sequence) into single BS }
  while Pos(#8' '#8, buf) > 0 do
    buf := StringReplace(buf, #8' '#8, #8, [rfReplaceAll]);
  ...
  try
    ...
    begin
      ...
      else if buf[i] = #8 then
      begin
        { Destructive backspace — remove last char from current line }
        if MemoTerminal.Lines.Count > 0 then
        begin
          s := MemoTerminal.Lines[MemoTerminal.Lines.Count - 1];
          if Length(s) > 0 then
            MemoTerminal.Lines[MemoTerminal.Lines.Count - 1] := Copy(s, 1, Length(s) - 1);
        end;
      end
      else
  ...
  { Position caret at end of actual content — only when memo is focused so
    we don't break the user's selection. Trim trailing newlines so cursor
    sits on the last content line, not an empty line below it. }
  if MemoTerminal.Focused then
  begin
    s := MemoTerminal.Text;
    i := Length(s);
    while (i > 0) and (s[i] in [#10, #13]) do Dec(i);
    MemoTerminal.SelStart  := i;
...
begin
  MemoTerminal.Color := clWindow;  //$00E8FFE8;   { pale green -- keyboard active }
end;

procedure TfrmMain.MemoTerminalExit(Sender: TObject);
begin
  MemoTerminal.Color := clBtnFace; //clWindow;
