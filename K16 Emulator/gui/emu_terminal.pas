...
begin
  case Key of
    ...
    { ---- k/OS Phase B foreground-switcher hot keys ---------------------
      Ctrl-N      → $0E (next shell)
      Ctrl-P      → $10 (prev shell) — but Ctrl-P is often eaten by host
                                       menu shortcuts (Print, etc), so
                                       Ctrl-Shift-N is also accepted as a
                                       fallback for prev.
      Ctrl-LeftArrow / Ctrl-RightArrow are mapped to $10 / $0E above. }
    Ord('N')  : if ssCtrl in Shift then
                begin
                  if ssShift in Shift then
                    begin if Assigned(FOnKey) then FOnKey($10); end  { Ctrl-Shift-N = prev }
                  else
                    begin if Assigned(FOnKey) then FOnKey($0E); end; { Ctrl-N = next }
                  Key := 0;
                end;
