...
const
  ...
  { op   m00 m01 m10 m11 }
  { $00 MISC   } ( 2,  2,  0,  0),   { NOP, HALT, -, - (NEG moved to $1E mode 01) }
  { $01 LOOKUP } ( 3,  3,  3,  3),
  { $02 INC/DEC} ( 5,  6,  0,  0),
  { $03 LEA    } ( 5,  5,  6,  5),   { mode 2 = PC-relative (6 cycles, has imm word) }
  ...
  { $0D OR     } ( 4,  4,  3,  4),
  { $0E XOR    } ( 4,  4,  3,  4),
  { $0F NOT    } ( 4,  4,  4,  4),   { NOT reg/mem/in-place/imm16 — all modes valid }
  { $10 CMP    } ( 3,  3,  3,  3),   { was (3,3,3,4) — all modes 3 cycles }
  { $11 Bcc    } ( 3,  4,  3,  4),
  { $12 JMP    } ( 2,  2,  4,  3),   { JMP24=2, JMP16=2, JMPT=4, JMPXY=3 }
  { $13 CALL   } (11, 11, 12, 10),   { CALL24=11, CALL16=11, CALLR=12, CALLXY=10 }
  { $14 LOADD  } ( 2,  3,  4,  3),
  { $15 LOADB  } ( 2,  3,  4,  3),
  { $16 LOADX  } ( 2,  3,  4,  3),
  { $17 LOADY  } ( 2,  3,  4,  3),
  ...
  { $1E TRAP/R } (12,  3,  6,  5),   { TRAP=12, NEG=3 (mode 01, was at $00.11), RETCC/RETCS=6, RET=5 }
...
begin
  ...
  case opcode of
    ...
    { RETCC/RETCS ($1E mode 10, v3.11): single-word, no extra word }
...
procedure Fetch(out D: TDecodedInstr);
begin
  { Code alignment fault — odd PC.  Raise the fault and return a NOP-shaped
    decode so the dispatcher executes a benign NOP this cycle; the outer
    loop sees CPU.Halted and exits before the next fetch. }
  if (CPU.PC and 1) <> 0 then
  begin
    if Assigned(CodeFaultHook) then CodeFaultHook(CPU.PC);
    D.Opcode  := $00;   { NOP }
    D.Mode    := 0;
    D.Operand := 0;
    D.HasImm  := False;
