...
// ---------------------------------------------------------------------------
// Alignment-fault helpers — wired into emu_mem via DataFaultHook,
// and invoked from Fetch on odd PC.
// One-shot: if CPU is already halted, do nothing (avoids cascading from
// secondary memory accesses inside the same dispatch).
// ---------------------------------------------------------------------------

procedure RaiseDataFault(addr: TAddr);
var
  msg : string;
  i   : Integer;
begin
  if CPU.Halted then Exit;
  msg := Format('DATA FAULT odd-addr word access $%6.6X PC=$%6.6X',
                [addr, CPU.PC and ADDR_MASK]);
  if Assigned(IO) then
  begin
    IO^.WriteByte(TERM_ADDR, 10);
    for i := 1 to Length(msg) do
  ...
  CPU.Halted   := True;
  CPU.HaltCode := $FD;   { $FD = data alignment fault }
end;

procedure RaiseCodeFault(addr: TAddr);
var
  msg : string;
  i   : Integer;
begin
  if CPU.Halted then Exit;
  msg := Format('CODE FAULT odd-addr fetch PC=$%6.6X', [addr]);
  if Assigned(IO) then
  begin
    IO^.WriteByte(TERM_ADDR, 10);
    for i := 1 to Length(msg) do
  ...
  CPU.Halted   := True;
  CPU.HaltCode := $FC;   { $FC = code alignment fault }
end;
...
{ INC XYn, #imm5 — XYn += imm5 (24-bit with carry into Y).
  CR-2026-001 v1.2 (FLAGSX): opcode $02 is now flag-transparent.
  Hardware writes the X-half carry to SRX (invisible), not SR.
  No SR update here — matches Reference Manual section 15.3 (— — — —). }
var
  n    : Byte;
  imm  : Byte;
  oldX : TWord;
...
begin
  ...
  CPU.XYSet(n, newXY);
  if oldX = 0 then ;  { suppress unused-var hint }
end;

procedure ExecDEC_Word(const D: TDecodedInstr);
{ DEC XYn, #imm5 — XYn -= imm5 (24-bit with borrow from Y).
  CR-2026-001 v1.2 (FLAGSX): opcode $02 is now flag-transparent.
  Hardware writes the X-half borrow to SRX (invisible), not SR.
  No SR update here — matches Reference Manual section 15.3 (— — — —). }
var
  n    : Byte;
  imm  : Byte;
...
begin
  ...
  if EvalCond(cond) then
    case Dd of
      0..3:  CPU.D[Dd]    := $FFFF;       { all bits set — K16 spec / Forth-83 true }
      4..7:  CPU.X[Dd-4]  := $FFFF;
      8..11: CPU.Y[Dd-8]  := $FF;         { Y is 8-bit }
    end
  else
    case Dd of
      0..3:  CPU.D[Dd]    := 0;
...
var
  ...
  begin
    case r of
      ...
      12:    Result := CPU.ORDB;
      13:    Result := CPU.SRToWord;     { SR / FLAGS }
  ...
  begin
    case r of
      ...
      12:    CPU.ORDB   := v;
      13:    CPU.SRFromWordFlagsOnly(v); { SR: only C/Z/N/V writable }
...
{ PUSH reg, XYsp - rf4 at bits 8:5 (D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11,
  ORDB=12, SR=13, PCH=14, PCL=15); sp=bits 2:1 }
...
begin
  ...
  case rf4 of
    ...
    8..11: v := CPU.Y[rf4-8];
    12:    v := CPU.ORDB;
    13:    v := CPU.SRToWord;        { SR / FLAGS }
    14:    v := CPU.PC shr 16;       { PCH }
    15:    v := CPU.PC and $FFFF;    { PCL }
  else     v := 0;
  end;
...
{ PUSH D123, XYsp — v3.12 — pushes D1,D2,D3 (D0 not touched).
  Stack layout after: [SP+0]=D3 (lowest), [SP+2]=D2, [SP+4]=D1 (highest).
  Microcode order: D1 pushed first (highest addr), then D2, then D3 (lowest addr).
  sp = IR[2:1] }
...
{ PUSH XYn, XYsp - IR[8:7]=XYn, IR[2:1]=sp; Y at lower addr, X at higher
  Per K16_PUSHPOP_Spec_V3_1.md MODE 10 "Pair Encoding (IR8-IR7, IR6-IR5=00)". }
var n, sp: Byte;
begin
...
{ POP reg, XYsp - rf4 at bits 8:5 (D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11, SR=13); sp=bits 2:1
  ORDB/PCH/PCL (rf4=12/14/15) POP not implemented here — PC write via POP needs
  careful handling of 24-bit address reassembly; add if a real use case arises. }
var rf4, sp: Byte; v: TWord;
begin
  rf4 := (D.Operand shr 5) and $0F;
  sp  := (D.Operand shr 1) and 3;
...
{ POP D123, XYsp — v3.12 — pops into D3,D2,D1 (D0 not touched).
  Microcode order: mem→D3 first (from lowest addr), then mem→D2, then mem→D1.
  Mirror image of PUSH D123: stack layout before pop is
    [SP+0]=D3 (lowest), [SP+2]=D2, [SP+4]=D1 (highest).
  Note: latent bug from the original POP D (per-xy_sel DBRead override
  routing the word to D0..D3 based on stack pointer) is dead — the new
  microcode sets DBRead=dbRead3 once outside the per-xy_sel switch, and
  this emulator uses the loop index as destination, matching that.
  sp = IR[2:1] }
var sp: Byte; i: Integer;
begin
  sp := (D.Operand shr 1) and 3;
...
{ POP XYn, XYsp - IR[8:7]=XYn, IR[2:1]=sp; Y at lower addr popped first, then X
  Per K16_PUSHPOP_Spec_V3_1.md MODE 10 "Pair Encoding (IR8-IR7, IR6-IR5=00)". }
var n, sp: Byte;
begin
  n  := (D.Operand shr 7) and 3;
...
{ LOADI reg, #imm5 - IR[8:5]=reg field (4 bits), IR[4:0]=imm5
  D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11, SR=13 }
var rf: Byte;
begin
  ...
  case rf of
    ...
    13:    CPU.SRFromWordFlagsOnly(D.Operand and $1F); { SR: sets C/Z/N/V; IE/LVL untouched }
  end;
end;
...
begin
  ...
  case rf of
    ...
    13:    CPU.SRFromWordFlagsOnly(D.Imm16);  { SR: only C/Z/N/V writable }
  end;
end;
...
{ LOADP/LOADPB/LOADZ/LOADZB - paged or zero-page load
  reg = rf4 at bits 8:5 (4-bit: D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11)
  bit 4 = ZOA flag: 0 = LOADP (Y-banked), 1 = LOADZ (page $00, Y don't-care)
  bit 3 = byte/word: 0 = word, 1 = byte
  bits 2:1 = Yn bank register (only used when ZOA=0) }
var rf4, Yn: Byte; addr: TAddr; v: TWord; isZOA: Boolean;
begin
  rf4   := (D.Operand shr 5) and $0F;
  isZOA := (D.Operand shr 4) and 1 = 1;
  if isZOA then
    addr := D.Imm16                                  { page $00 }
  else
  begin
...
{ STOREP/STOREPB/STOREZ/STOREZB - paged or zero-page store
  reg = rf4 at bits 8:5 (4-bit: D0-D3=0-3, X0-X3=4-7, Y0-Y3=8-11)
  bit 4 = ZOA flag: 0 = STOREP (Y-banked), 1 = STOREZ (page $00, Y don't-care)
  bit 3 = byte/word: 0 = word, 1 = byte
  bits 2:1 = Yn bank register (only used when ZOA=0) }
var rf4, Yn: Byte; addr: TAddr; v: TWord; isZOA: Boolean;
begin
...
{ TRAP #n — vector at physical [$00:n*4] via ZOA (Y-independent).
  Entry layout: page_word at +0 (low byte = high byte of handler PC),
                addr_word at +2 (low 16 bits of handler PC).
  IR encoding: IR[7:0] = n*2, so n = (IR[7:0] shr 1).
  k/OS v3.10: vector table now lives at physical $000000–$0001FC and
  is reached via ABWriteZOA, decoupling dispatch from the running task's Y3. }
var
  n        : Byte;
  vecBase  : TAddr;
  pageWord : TWord;
...
begin
  ...
  vecBase  := TAddr(n * 4);                  { ZOA: AB-hi = $00 }
  pageWord := MemReadWord(vecBase);          { low byte = target PC[23:16] }
  addrWord := MemReadWord(vecBase + 2);      { target PC[15:0] }
  CPU.StackPush24(CPU.PC);
...
{ RET [#nw] - IMM5 in IR[4:0] (D.Operand bits [4:0])
  ...
  SR unaffected (no FLAGSLoad in step 5). }
var
  imm5    : Byte;
  cleanup : TWord;
begin
...
procedure ExecRETCC_RETCS(const D: TDecodedInstr);
{ RETCC / RETCS [#nw] — v3.11, mode 10 in opcode $1E.
  Discriminated by IR[8:7]:
    00 → RETCC (clear carry)
    01 → RETCS (set carry)
    10, 11 → reserved; fall through to RETCC behaviour (matches microcode).
  Steps 1-4 identical to RET (pop return addr + IMM5 cleanup).
  Step 5 runs ALU as flag generator with FLAGSLoad asserted, cmADD:
    RETCC: ALU-A=$0000 + ALU-B=$0002 = $0002 → C=0, Z=0, N=0, V=0  (SR=$00)
    RETCS: ALU-A=$FFFF + ALU-B=$0002 = $10001 → C=1, Z=0, N=0, V=0  (SR=$01)
  The +$0002 (not +$0001) is deliberate per microcode spec section 6: it
  makes the low-16 result non-zero for both variants, so Z=0 in both
  cases. Behaviourally identical to LOADI SR,#$00/#$01 + RET — only C
  differs between RETCC and RETCS, all other flags forced to 0.
  IMM5 cleanup handling identical to RET. }
var
  imm5    : Byte;
  cleanup : TWord;
  sel     : Byte;
begin
  { Steps 1-4: pop return address + cleanup (same as RET) }
  CPU.PC := CPU.StackPop24;
  imm5   := D.Operand and $1F;
  if imm5 > 4 then
  begin
  ...
  { Step 5: deterministic SR write.
    sel = IR[8:7] = (operand shr 7) and 3.
    sel=00 RETCC (SR←$00); sel=01 RETCS (SR←$01); sel=10/11 fall through to RETCC. }
  sel := (D.Operand shr 7) and 3;
  CPU.SR.Flags.Z := False;
  CPU.SR.Flags.N := False;
  CPU.SR.Flags.V := False;
  CPU.SR.Flags.C := sel = 1;   { RETCS only }
end;
...
{ Hardware interrupt — opcode $FFFF forced onto IR by hardware (or by cpu_thread
  ...
  Vector: 3-byte address at physical [$00:0000] (page byte) and [$00:0002] (low word)
  via ABWriteZOA — independent of the running task's Y3.
...
begin
  ...
  { Read 3-byte handler address from INT vector at physical $000000 (ZOA) }
  VecBase    := $000000;
  CPU.T8     := MemReadWord(VecBase)       and $FF;   { page byte at $000000 }
  CPU.PC     := (TAddr(CPU.T8) shl 16)
              or MemReadWord(VecBase + 2);             { low word at $000002 }
end;
...
var
  op, md: Integer;
begin
  { Wire alignment-fault hooks into emu_mem }
  DataFaultHook := @RaiseDataFault;
  CodeFaultHook := @RaiseCodeFault;
  ...
  { $00 mode 3 = illegal (NEG relocated to $1E mode 01 by CR-2026-001 v1.2;
    its flag writes would have been suppressed under FLAGSX, breaking the
    arithmetic contract). }
  ...
  { $1E TRAP/RET/NEG — mode 01 = NEG (relocated by CR-2026-001 v1.2);
    mode 10 = RETCC/RETCS (v3.11) }
  DispatchTable[$1E, 0] := @ExecTRAP;
  DispatchTable[$1E, 1] := @ExecNEG;
  DispatchTable[$1E, 2] := @ExecRETCC_RETCS;
