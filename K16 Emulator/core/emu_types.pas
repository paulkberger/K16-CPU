...
const
  ...
  RAM_BASE   = $000000;
  RAM_TOP    = $AFFFFF;                  { RAM + expansion — v4 }

  FB_BASE_DEFAULT = $B00000;             { default FB base (page $00B0 << 16) }
  VRAM_TOP   = $CFFFFF;

  IO_BASE    = $D80000;                  { I/O region — v4 (was $C00000) }
  IO_TOP     = $DFFFFF;
  VID_PAGE   = $DC0000;                  { FB page register — word, zero-extended; default $00B0 }
  VID_MODE   = $DD0000;                  { video mode register — word write }
  KBD_ADDR   = $DE0000;                  { keyboard input — word read }
  TERM_ADDR  = $DF0000;                  { terminal output — byte write }
...
var
  ...
  BreakAddr        : TAddr      = 0;
  FB_BASE          : TAddr      = FB_BASE_DEFAULT;  { current FB base, written via VID_PAGE }
  { VideoMode and FrameDirty live in emu_mem — authoritative there }
