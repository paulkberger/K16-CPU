...
begin
  ...
  case op of
    ...
    $06: { PUSH }
      begin
        ...
        case mode of
          ...
          1: Result := Result + Format('PUSH D123, %s',[XYName[Xs]]);
    ...
    $07: { POP }
      begin
        ...
        case mode of
          ...
          1: Result := Result + Format('POP D123, %s',[XYName[Xs]]);
    ...
    $18: { LOADI }
      begin
        ...
        case mode of
          ...
          3: begin imm := MemReadWord(addr+2); BytesUsed := 4;
               if (opr shr 4) and 1 = 1 then
               begin
                 { ZOA variant: LOADZ / LOADZB }
                 if (opr shr 3) and 1 = 1 then
                   Result := Result + Format('LOADZB %s, [#$%4.4X]',
                     [Rf4Name(rf4), imm])
                 else
                   Result := Result + Format('LOADZ %s, [#$%4.4X]',
                     [Rf4Name(rf4), imm]);
               end
               else if (opr shr 3) and 1 = 1 then
    ...
    $1D: { STOREI }
      case mode of
        ...
        3: begin imm := MemReadWord(addr+2); BytesUsed := 4;
             if (opr shr 4) and 1 = 1 then
             begin
               { ZOA variant: STOREZ / STOREZB }
               if (opr shr 3) and 1 = 1 then
                 Result := Result + Format('STOREZB %s, [#$%4.4X]',
                   [Rf4Name((opr shr 5) and $0F), imm])
               else
                 Result := Result + Format('STOREZ %s, [#$%4.4X]',
                   [Rf4Name((opr shr 5) and $0F), imm]);
             end
             else if (opr shr 3) and 1 = 1 then
    ...
    $1E: { TRAP / RETCC / RETCS / RET }
      if mode = 0 then
        Result := Result + Format('TRAP #%d',[(opr and $FF) shr 1])
      else if mode = 2 then
      begin
        { mode 10: IR[8:7] selects RETCC (00) / RETCS (01) / reserved (10,11→RETCC) }
        if ((opr shr 7) and 3) = 1 then
        begin
          if opr and $1F = 4 then Result := Result + 'RETCS'
          else Result := Result + Format('RETCS #%dw',[(opr and $1F) - 4]);
        end
        else
        begin
          if opr and $1F = 4 then Result := Result + 'RETCC'
          else Result := Result + Format('RETCC #%dw',[(opr and $1F) - 4]);
        end;
      end
      else if mode = 3 then
      begin
