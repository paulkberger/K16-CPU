...
{ BRA short — mode 2, unconditional. Cond field is don't-care; must NOT call EvalCond. }
var off: TAddr;
begin
  off := D.Operand and $1F;
  CPU.PC := (CPU.PC + off) and ADDR_MASK;
...
{ JMPT XYn, Dm — indexed indirect jump through table.
  Encoding: D at IR[8:7], XY at IR[6:5].
  EA = XYn + Dm; PC = (Yn << 16) | Mem[EA] }
var n, dn: Byte; ea: TAddr;
begin
  dn := (D.Operand shr 7) and 3;   { D register — IR bits 8:7 }
  n  := (D.Operand shr 5) and 3;   { XY register — IR bits 6:5 }
  ea := (CPU.XYGet(n) + CPU.D[dn]) and ADDR_MASK;
  CPU.PC := (TAddr(CPU.Y[n]) shl 16) or MemReadWord(ea);
end;
...
begin
  ...
  { $11 Bcc - mode 2 = BRA short (unconditional) }
  DispatchTable[$11, 0] := @ExecBcc_Short;
  DispatchTable[$11, 1] := @ExecBcc_Long;
  DispatchTable[$11, 2] := @ExecBRA_Short;      { unconditional — was ExecBcc_Short, wrong }
