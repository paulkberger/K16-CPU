...
{ RETCC / RETCS [#nw] — v3.11, mode 10 in opcode $1E.
  ...
  Step 5 runs ALU as flag generator with FLAGSLoad asserted, cmADD:
    RETCC: ALU-A=$0000 + ALU-B=$0001 = $0001 → C=0, Z=0, N=0, V=0
    RETCS: ALU-A=$FFFF + ALU-B=$0001 = $10000 → C=1, Z=1, N=0, V=0
  All four flags are written by FLAGSLoad — RETCS sets Z as a side effect.
...
begin
  ...
  { Step 5: ALU flag-generator side effect.
    sel = IR[8:7] = (operand shr 7) and 3.
    sel=00 RETCC; sel=01 RETCS; sel=10/11 fall through to RETCC. }
  sel := (D.Operand shr 7) and 3;
  if sel = 1 then
  begin
    { RETCS: $FFFF + $0001 = $10000 }
    CPU.SR.Flags.C := True;
    CPU.SR.Flags.Z := True;
  ...
  begin
    { RETCC (and reserved encodings): $0000 + $0001 = $0001 }
...
begin
  ...
  { $1E TRAP/RET — mode 01 spare, mode 10 = RETCC/RETCS (v3.11) }
