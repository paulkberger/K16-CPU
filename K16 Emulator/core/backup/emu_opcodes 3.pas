...
{ MOVE/SWAP - mode selects operation, rf4 fields select registers
  Mode 0: MOVE dst(rf4 bits 8:5), Dn (src=D reg bits 4:3)
  Mode 1: MOVE dst(rf4 bits 8:5), src(rf4 bits 4:1)
...
begin
  ...
  case D.Mode of
    0: begin  { MOVE dst, Dn — src is D reg in bits 4:3 }
         src := (D.Operand shr 3) and 3;
         WriteRf4(dst, CPU.D[src]);
       end;
...
{ ALU Dd, [XYn] - uses memory at XY as source }
var Dd, Xs: Byte;
begin
  Dd := (D.Operand shr 7) and 3;
  Xs := (D.Operand shr 5) and 3;
  CPU.D[Dd] := DoAluOp(op, CPU.D[Dd], MemReadWord(CPU.XYGet(Xs)));
end;
...
{ RET [#nw] - IMM5 in IR[5:1]; base IMM5=4 means no cleanup
  Extra stack cleanup = IMM5 - 4 bytes = nw*2 bytes }
var
  imm5    : Byte;
  cleanup : TWord;
begin
  CPU.PC := CPU.StackPop24;
  imm5   := (D.Operand shr 1) and $1F;
  if imm5 > 4 then
  begin
    cleanup := imm5 - 4;
