  ...
  case op of
    $00: { MISC }
      case mode of
        ...
        3: begin
             Dd := (opr shr 7) and 3; Ds := (opr shr 5) and 3;
             if Dd = Ds then Result := Result + Format('NEG %s', [DnName[Dd]])
             else            Result := Result + Format('NEG %s, %s', [DnName[Dd], DnName[Ds]]);
           end;
    ...
    $01: { LOOKUP }
      begin
        Dd := (opr shr 5) and 3;
        case opr and $1F of
          $01: mnem:='SHL';   $02: mnem:='SHR';   $03: mnem:='ASR';
          $04: mnem:='ROL';   $05: mnem:='ROR';   $06: mnem:='SWAPB';
          $07: mnem:='HIGH';  $08: mnem:='LOW';   $09: mnem:='SHL4';
          $0A: mnem:='SHR4';  $0B: mnem:='ASR4';  $0C: mnem:='ASR8';
          $0D: mnem:='MULB';  $0E: mnem:='RECIP';
        else   mnem:=Format('LOOKUP#$%2.2X',[opr and $1F]);
        end;
    ...
    $03: { LEA }
      begin
        ...
        case mode of
          0: Result := Result + Format('LEA %s, %s+#%d',[XYName[Dd],XYName[Xs],(opr and $1F)*2]);
          1: Result := Result + Format('LEA %s, %s+D%d',[XYName[Dd],XYName[Xs],(opr shr 3) and 3]);
    ...
    $04: { Scc }
      begin
        Dd:=(opr shr 7) and 3; cond:=(opr shr 2) and 7;
        Result := Result + Format('S%s %s',[CcName[cond],DnName[Dd]]);
      end;

    $05: { MOVE / SWAP }
      begin
        case mode of
          0: begin { MOVE dst(4bit), Dn — src is D reg in bits 4:3 }
               rf4 := (opr shr 5) and $0F;
               Dd  := (opr shr 3) and 3;
               Result := Result + Format('MOVE %s, %s',[Rf4Name(rf4), DnName[Dd]]);
             end;
    ...
    $06: { PUSH }
      begin
        Xs := (opr shr 1) and 3;
        case mode of
          0: Result := Result + Format('PUSH %s, %s',[DnName[(opr shr 7) and 3],XYName[Xs]]);
          1: Result := Result + Format('PUSH D-grp, %s',[XYName[Xs]]);
          2: Result := Result + Format('PUSH %s, %s',[XYName[(opr shr 5) and 3],XYName[Xs]]);
          3: Result := Result + Format('PUSH #%d, %s',[opr and $1F,XYName[Xs]]);
        end;
      end;
    ...
    $07: { POP }
      begin
        Xs := (opr shr 1) and 3;
        case mode of
          0: Result := Result + Format('POP %s, %s',[DnName[(opr shr 7) and 3],XYName[Xs]]);
    ...
    $08,$09,$0A,$0B,$0C,$0D,$0E,$0F,$10: { ALU + CMP }
      begin
        Dd:=(opr shr 7) and 3; Ds:=(opr shr 5) and 3; li:=op-$08;
        case mode of
          0: Result := Result + Format('%s %s, %s',      [AluName[li],DnName[Dd],DnName[Ds]]);
          1: Result := Result + Format('%s %s, %s',      [AluName[li],DnName[Dd],XnName[Ds]]);
          2: Result := Result + Format('%s %s, #%d',     [AluName[li],DnName[Dd],opr and $1F]);
          3: begin imm:=MemReadWord(addr+2); BytesUsed:=4;
               Result := Result + Format('%s %s, #$%4.4X',[AluName[li],DnName[Dd],imm]); end;
        end;
      end;
    ...
    $12: { JMP }
      case mode of
        ...
        2: Result := Result + Format('JMPT %s', [XYName[(opr shr 5) and 3]]);
    ...
    $18: { LOADI }
      begin
        ...
        rf4 := (opr shr 5) and $0F;
        case rf4 of
          0..3:  regname := DnName[rf4];
          4..7:  regname := XnName[rf4-4];
          8..11: regname := Format('Y%d',[rf4-8]);
        else     regname := Format('?%d',[rf4]);
        end;
        case mode of
          0: Result := Result + Format('LOADI %s, #%d',[regname, opr and $1F]);
          1: begin imm:=MemReadWord(addr+2); BytesUsed:=4;
               Result := Result + Format('LOADI %s, #$%4.4X',[regname, imm]); end;
          2: begin imm:=MemReadWord(addr+2); BytesUsed:=6;
               Result := Result + Format('LOADXY %s, #$%2.2X%4.4X',
                 [XYName[Dd],imm and $FF,MemReadWord(addr+4)]); end;
          3: Result := Result + Format('LOADP %s, [X%d]',[DnName[Dd],Xs]);
        end;
      end;
    ...
    $1D: { STOREI }
      case mode of
        0: Result := Result + Format('STOREI D%d, [%s+#%d]',
             [(opr shr 7) and 3,XYName[(opr shr 5) and 3],opr and $1F]);
        1: begin imm:=MemReadWord(addr+2); BytesUsed:=4;
        ...
        3: Result := Result + Format('STOREP D%d, [X%d]',
             [(opr shr 7) and 3,(opr shr 5) and 3]);
      end;
