...
function AluNeg(a: TWord): TWord;   { NEG: result = 0 - a; sets Z and N only }
...
begin
  ...
  { NEG sets Z and N only — not C or V }
  CPU.SR.Flags.Z := Result = 0;
  CPU.SR.Flags.N := (Result and $8000) <> 0;
end;
...
function LookupSHL(v: TWord): TWord;
begin
  CPU.SR.Flags.C := (v and $8000) <> 0;
  Result := (v shl 1) and $FFFF;
end;

function LookupSHR(v: TWord): TWord;
begin
  CPU.SR.Flags.C := (v and $0001) <> 0;
  Result := v shr 1;
end;
...
begin
  oldC := CPU.SR.Flags.C;
  CPU.SR.Flags.C := (v and $8000) <> 0;
  Result := ((v shl 1) and $FFFF) or (Ord(oldC) and 1);
end;
...
begin
  oldC := CPU.SR.Flags.C;
  CPU.SR.Flags.C := (v and $0001) <> 0;
  Result := (v shr 1) or (TWord(Ord(oldC)) shl 15);
end;
