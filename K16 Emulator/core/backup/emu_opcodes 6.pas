...
{ JMPT XYn, Dm — indexed indirect jump table (3 cycles)
  Encoding: IR[6:5]=XYn, IR[7:6]=Dm  (per microcode: dd=bits7:6, xy=bits5:4 of opr)
  Operation: EA = Xn + Dm; PC[15:0] = mem[Yn:EA]; PC[23:16] = Yn }
var
  n  : Byte;
  m  : Byte;
  EA : TAddr;
begin
  n  := (D.Operand shr 4) and 3;   { XY pair: bits 5:4 of operand }
  m  := (D.Operand shr 6) and 3;   { D reg:   bits 7:6 of operand }
...
begin
  ...
  DispatchTable[$13, 2] := @ExecCALLXY;  { mode 10 = CALLXY per microcode/ROM }
  DispatchTable[$13, 3] := @ExecCALLR;   { mode 11 = CALLR  per microcode/ROM }
