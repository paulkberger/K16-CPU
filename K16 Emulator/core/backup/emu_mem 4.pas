unit emu_mem;
{
  ...
  I/O range $C00000..$DFFFFF is routed through the IO^ handler.
...
var
  ...
  VideoMode  : TWord    = 0;   { current video mode, maintained by IODispatchWrite }
  FrameDirty : Boolean  = False; { set when FB written or mode changed; cleared by UI }
...
begin
  ...
  Mem[a] := v;
  if (a >= FB_BASE) and (a < FB_BASE + $4B000) then
    FrameDirty := True;
end;
